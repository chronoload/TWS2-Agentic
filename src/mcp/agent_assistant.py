    ...
    def _restore_conversation_to_agent(self, conversation):
        ...
        for msg in conversation.messages:
            ...
            if msg.get("tool_args"):
                message["tool_args"] = msg["tool_args"]
            if msg.get("tool_result"):
                message["tool_result"] = msg["tool_result"]
...
"""
    ...
    def _truncate_tool_result(self, tool_result: Any) -> str:
        """专门用于截断工具返回结果，提升渲染性能 - 限制约3000字符 - 兼容非字符串类型"""
        if not tool_result:
            return ""
    ...
    def _new_conversation(self):
        ...
        if self._session_mgr:
            self._session_mgr.create_session("新对话")
            self._session_mgr.set_active_session(self.current_conversation_id)
    ...
    def _load_conversation(self, conversation_id: str):
        ...
        # 获取或创建对话专用的 Agent 实例
        if self._session_mgr:
            self._session_mgr.set_active_session(conversation_id)
        ...
        if self.agent and hasattr(self.agent, 'messages') and agent_messages_to_add:
            self.agent.messages.extend(agent_messages_to_add)
