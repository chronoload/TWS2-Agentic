...
class CourseTrackerApp:
    ...
    def _build_ui(self):
        ...
        self.nav_canvas = tk.Canvas(nav_container, height=42, highlightthickness=0)
        self.nav_canvas.pack(side=tk.TOP, fill=tk.X)

        self._nav_scrollbar = ttk.Scrollbar(
            nav_container, orient=tk.HORIZONTAL, command=self.nav_canvas.xview
        )

        self.nav_frame = ttk.Frame(self.nav_canvas)
        nav_window = self.nav_canvas.create_window((0, 0), window=self.nav_frame, anchor=tk.NW)

        def _on_nav_frame_configure(event):
            self.nav_canvas.configure(scrollregion=self.nav_canvas.bbox("all"))
            frame_width = self.nav_frame.winfo_reqwidth()
            canvas_width = self.nav_canvas.winfo_width()
            if frame_width > canvas_width:
                self.nav_canvas.itemconfigure(nav_window, width=frame_width)

        self.nav_frame.bind("<Configure>", _on_nav_frame_configure)

        def _on_nav_canvas_configure(event):
            frame_width = self.nav_frame.winfo_reqwidth()
            if frame_width < event.width:
        ...
        def _on_nav_mousewheel(event):
            self.nav_canvas.xview_scroll(int(-1 * (event.delta / 120)), "units")
        ...
        self._bind_nav_mousewheel_recursive = _bind_mousewheel_recursive

        def _on_nav_enter(event):
            self._nav_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)

        def _on_nav_leave(event):
            try:
                self._nav_scrollbar.pack_forget()
            except Exception:
                pass

        self.nav_canvas.bind("<Enter>", _on_nav_enter)
        self.nav_canvas.bind("<Leave>", _on_nav_leave)
        ...
        # 绑定所有导航组件的鼠标滚轮事件
        if hasattr(self, '_bind_nav_mousewheel_recursive'):
            self._bind_nav_mousewheel_recursive(self.nav_frame)
    ...
    def _show_course_detail(self, course_id: str):
        ...
        # 记录已加载的标签页
        self._course_detail_loaded_tabs = set()
        
        # 创建标签页，但先不构建内容
        tab_info = [
            ("📋 课时列表", self._build_lessons_list, (course, cid)),
            ("📚 Section结构", self._build_sections_view, (course, cid)),
            ("ℹ️ 课程信息", self._build_course_info, (course,)),
            ("🔗 课程资源", self._build_course_resources, (cid,)),
            ("📝 课程笔记", self._build_course_notes_tab, (cid,)),
        ]
        
        # 存储每个标签页的信息
        self._course_detail_tabs = []
        
        for tab_name, build_func, args in tab_info:
            tab_frame = ttk.Frame(notebook)
            notebook.add(tab_frame, text=tab_name)
            self._course_detail_tabs.append((tab_frame, build_func, args))
        
        # 默认加载第一个标签页
        self._load_course_detail_tab(0)
        
        # 绑定标签页切换事件
        def on_tab_changed(event):
            tab_index = notebook.index(notebook.select())
            self._load_course_detail_tab(tab_index)
        
        notebook.bind("<<NotebookTabChanged>>", on_tab_changed)
    
    def _load_course_detail_tab(self, tab_index):
        """加载指定索引的课程详情标签页（延迟加载）"""
        if tab_index in self._course_detail_loaded_tabs:
            return
        if hasattr(self, '_course_detail_tabs') and tab_index < len(self._course_detail_tabs):
            tab_frame, build_func, args = self._course_detail_tabs[tab_index]
            build_func(tab_frame, *args)
            self._course_detail_loaded_tabs.add(tab_index)
    ...
    def _open_notes_folder(self, course_id, lesson_number=None):
        """打开课程笔记所在文件夹"""
        path, created = self.system.get_or_create_note(course_id, lesson_number)
        if path:
            folder = path.parent
            if folder.exists():
                ...
                messagebox.showerror("错误", f"笔记文件夹不存在: {folder}")
        else:
            messagebox.showerror("错误", "无法获取笔记路径")
        ...
        def save_solution():
            ...
            try:
                ...
                if old_solution:
                    ...
                    # 直接计算问题结束位置（不需要重新解析）
                    # 问题_env的长度就是替换后新内容中问题结束的位置相对于start_replace的偏移
                    problem_env_length = len(problem_env)
                    insert_pos = start_replace + problem_env_length
        ...
        def save_proof():
            ...
            try:
                ...
                if old_proof:
                    ...
                    # 直接计算定理结束位置（不需要重新解析）
                    # 定理_env的长度就是替换后新内容中定理结束的位置相对于start_replace的偏移
                    theorem_env_length = len(theorem_env)
                    insert_pos = start_replace + theorem_env_length
...
# ============================================================
    ...
    def _show_search_page(self):
        ...
        ttk.Button(switch_bar, text="📥 从枢纽导入书签",
                   command=self._import_from_hub_bookmarks, width=16).pack(side=tk.LEFT, padx=3)
        ttk.Button(switch_bar, text="📥 从枢纽导入URL到爬虫",
                   command=self._import_from_hub_crawler, width=18).pack(side=tk.LEFT, padx=3)
        ttk.Button(switch_bar, text="🔍 发现订阅源",
                   command=self._discover_subscriptions_from_search, width=14).pack(side=tk.LEFT, padx=3)
    ...
    def _build_bookmark_section(self, parent):
        ...
        ttk.Button(header_frame, text="📥 从数据枢纽导入", command=self._import_from_hub_bookmarks).pack(side=tk.RIGHT, padx=5)
        ...
        self._bind_scroll_to_children = lambda: self._recursive_bind_scroll(self._bookmark_inner, on_mousewheel, on_mousewheel_linux)

        # 刷新书签
        self._refresh_bookmarks()

    def _recursive_bind_scroll(self, widget, on_mousewheel, on_mousewheel_linux):
        for child in widget.winfo_children():
            child.bind("<MouseWheel>", on_mousewheel)
            child.bind("<Button-4>", on_mousewheel_linux)
            child.bind("<Button-5>", on_mousewheel_linux)
            self._recursive_bind_scroll(child, on_mousewheel, on_mousewheel_linux)
    ...
    def _refresh_bookmarks(self):
        ...
        self._all_filtered_bookmarks = filtered
        self._bm_display_count = 18
        self._render_bookmark_page()

    def _render_bookmark_page(self):
        for widget in self._bookmark_inner.winfo_children():
            widget.destroy()

        filtered = getattr(self, '_all_filtered_bookmarks', [])
        display_count = getattr(self, '_bm_display_count', 18)
        ...
        if len(filtered) > display_count:
            more_row = row + 1
            more_frame = ttk.Frame(self._bookmark_inner)
            more_frame.grid(row=more_row, column=0, columnspan=max_cols, pady=8)
            ttk.Label(more_frame, text=f"已显示 {display_count}/{len(filtered)}  ",
                     foreground="#999", font=("", 9)).pack(side=tk.LEFT)
            ttk.Button(more_frame, text=f"加载更多 ({len(filtered) - display_count}项)",
                      command=self._load_more_bookmarks, width=20).pack(side=tk.LEFT)
    ...
    def _select_bookmarks_to_import(self, imported_bookmarks):
        """选择要导入的书签（支持搜索筛选）"""
        dlg = tk.Toplevel(self.root)
        dlg.title(f"选择要导入的书签 ({len(imported_bookmarks)}个)")
        dlg.geometry("750x550")
        dlg.transient(self.root)
        dlg.grab_set()
        ...
        ttk.Entry(search_row, textvariable=filter_var, width=40).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ttk.Label(search_row, text=f"共 {len(imported_bookmarks)} 个书签").pack(side=tk.RIGHT, padx=5)
        ...
        for i, bm in enumerate(imported_bookmarks):
            iid = tree.insert("", tk.END, values=(bm.get("name", ""), bm.get("url", "")), tags=(i,))
        ...
        def on_filter(*_):
            ...
            for iid in all_items:
                idx = int(tree.item(iid, "tags")[0])
                bm = imported_bookmarks[idx]
                if not text or text in bm.get("name", "").lower() or text in bm.get("url", "").lower():
                    tree.reattach(iid, "", 0)
                else:
                    tree.detach(iid)
        ...
        sel_row.pack(fill=tk.X, padx=10)
        ttk.Button(sel_row, text="全选", command=select_all, width=6).pack(side=tk.LEFT, padx=2)
        ttk.Button(sel_row, text="取消全选", command=deselect_all, width=8).pack(side=tk.LEFT, padx=2)
    ...
    def _show_bookmark_detail(self, bookmark):
        ...
        # 记录已加载的标签页
        self._bookmark_detail_loaded_tabs = set()
        
        # 创建标签页，但先不构建内容
        tab_info = [
            ("📥 下载文件夹", self._build_bookmark_downloads_tab, (bookmark_dir,)),
            ("📝 笔记文件夹", self._build_bookmark_notes_tab, (bookmark_dir,)),
            ("✏️ 快速笔记", self._build_bookmark_quick_note_tab, (bookmark_dir, bookmark)),
            ("ℹ️ 信息", self._build_bookmark_info_tab, (bookmark,)),
        ]
        
        # 存储每个标签页的信息
        self._bookmark_detail_tabs = []
        
        for tab_name, build_func, args in tab_info:
            tab_frame = ttk.Frame(notebook, padding=10)
            notebook.add(tab_frame, text=tab_name)
            self._bookmark_detail_tabs.append((tab_frame, build_func, args))
        
        # 默认加载第一个标签页
        self._load_bookmark_detail_tab(0)
        
        # 绑定标签页切换事件
        def on_tab_changed(event):
            tab_index = notebook.index(notebook.select())
            self._load_bookmark_detail_tab(tab_index)
        
        notebook.bind("<<NotebookTabChanged>>", on_tab_changed)
    
    def _load_bookmark_detail_tab(self, tab_index):
        """加载指定索引的书签详情标签页（延迟加载）"""
        if tab_index in self._bookmark_detail_loaded_tabs:
            return
        if hasattr(self, '_bookmark_detail_tabs') and tab_index < len(self._bookmark_detail_tabs):
            tab_frame, build_func, args = self._bookmark_detail_tabs[tab_index]
            build_func(tab_frame, *args)
            self._bookmark_detail_loaded_tabs.add(tab_index)
...
"""
    ...
    def _clear_search_right(self):
        ...
        main = ttk.Frame(self._search_right_frame, padding=10)
        main.pack(fill=tk.BOTH, expand=True)

        ttk.Label(main, text="📊 书签统计", font=("", 14, "bold")).pack(anchor="w", pady=(0, 10))
        ...
        cat_names = {"preprint": "预印本", "search": "学术搜索", "journal": "期刊",
                     "database": "数据库", "tool": "工具", "other": "其他"}
        for cat_key, cat_name in cat_names.items():
            count = cat_count.get(cat_key, 0)
            if count > 0:
                ttk.Label(main, text=f"  • {cat_name}: {count} 个", font=("", 10)).pack(anchor="w")

        ttk.Label(main, text=f"\n📦 总计: {len(bookmarks)} 个书签", font=("", 10, "bold")).pack(anchor="w")
        
        ttk.Separator(main, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=12)
        
        # 分析工具入口
        ttk.Label(main, text="🔍 分析工具", font=("", 12, "bold")).pack(anchor="w", pady=(0, 8))
        
        analyze_btn_frame = ttk.Frame(main)
        analyze_btn_frame.pack(fill=tk.X)
        
        ttk.Button(analyze_btn_frame, text="📝 分析所有书签标题", 
                   command=self._analyze_all_bookmark_titles).pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        ttk.Button(analyze_btn_frame, text="🔗 检查失效链接", 
                   command=self._check_broken_links).pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
        
        ttk.Button(main, text="📊 生成书签分类报告", 
                   command=self._generate_bookmark_report).pack(fill=tk.X, pady=2)
    ...
    def _analyze_all_bookmark_titles(self):
        """分析所有书签标题，提取关键词和主题"""
        bookmarks = self._load_bookmarks()
        if not bookmarks:
            messagebox.showinfo("提示", "没有书签数据")
            return
        
        titles = [bm.get("name", "") for bm in bookmarks if bm.get("name")]
        ...
        text = tk.Text(dlg, wrap=tk.WORD, font=("", 10))
    ...
    def _check_broken_links(self):
            ...
            for bm in bookmarks:
                ...
                try:
                    ...
                    if response.getcode() == 200:
                        result_text.append(f"✅ {bm.get('name', '未知')} - {url[:50]}...")
                    else:
                        result_text.append(f"⚠️ {bm.get('name', '未知')} - HTTP {response.getcode()}")
                except Exception as e:
                    result_text.append(f"❌ {bm.get('name', '未知')} - {str(e)[:30]}")
        ...
        def update_ui():
            ...
            text.insert(tk.END, "\n".join(result_text))
            text.config(state=tk.DISABLED)
            status_label.config(text=f"已检查 {checked_count[0]} 个链接")
        ...
        status_label = ttk.Label(dlg, text="检查中...")
        status_label.pack(pady=5)
        
        text = tk.Text(dlg, wrap=tk.WORD, font=("", 10))
        text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        text.insert(tk.END, "正在检查链接...\n")
        text.config(state=tk.DISABLED)
        
        check_btn = ttk.Button(dlg, text="关闭", command=dlg.destroy, state=tk.DISABLED)
        check_btn.pack(pady=10)
        
        # 启动后台检查
        threading.Thread(target=check_links, daemon=True).start()

    def _generate_bookmark_report(self):
        """生成书签分类报告"""
        bookmarks = self._load_bookmarks()
        if not bookmarks:
            messagebox.showinfo("提示", "没有书签数据")
        ...
        # 按分类统计
        from collections import Counter
        cat_counts = Counter(bm.get("category", "other") for bm in bookmarks)
        total = len(bookmarks)
        ...
        for cat, count in sorted(cat_counts.items(), key=lambda x: -x[1]):
            ...
            report += f"  {cat_name}: {count} ({percentage:.1f}%) {bar}\n"
        ...
        def save_report():
            filepath = filedialog.asksaveasfilename(
                ...
                initialfile=f"bookmark_report_{datetime.now().strftime('%Y%m%d')}.txt"
            )
            if filepath:
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(report)
                messagebox.showinfo("保存成功", f"报告已保存到: {filepath}")
        
        btn_frame = ttk.Frame(dlg)
        btn_frame.pack(pady=10)
        ttk.Button(btn_frame, text="保存报告", command=save_report).pack(side=tk.LEFT, padx=5)
...
"""
    ...
    def _build_project_file_browser(self, parent, project):
        ...
        count_label = ttk.Label(search_frame, textvariable=search_count_var, font=("Microsoft YaHei UI", 8))
        count_label.pack(side=tk.LEFT, padx=5)
        
        # 清除搜索按钮
        def clear_search():
            search_var.set("")
            search_type_var.set("名称")
            perform_search()
        
        clear_btn = ttk.Button(search_frame, text="✕", command=clear_search, width=3)
...
'''
        ...
        def show_context_menu(event):
            ...
            if full_path and full_path.is_dir():
                menu.add_command(label="📂 打开文件夹", command=lambda: open_file(str(full_path)))
    ...
    def _show_research_analysis(self):
        ...
        ttk.Button(header_frame, text="📥 从数据枢纽导入", command=self._import_from_hub_analysis).pack(side=tk.RIGHT, padx=5)
        ...
        # 链接解析标签页
        links_frame = ttk.Frame(self._research_notebook, padding=10)
        self._research_notebook.add(links_frame, text="🔗 链接解析")
        
        links_toolbar = ttk.Frame(links_frame)
        links_toolbar.pack(fill=tk.X, pady=(0, 5))
        
        self._import_links_btn = ttk.Button(links_toolbar, text="📥 导入链接到数据中心", 
                                            command=self._import_links_to_hub, state=tk.DISABLED)
        self._import_links_btn.pack(side=tk.LEFT, padx=2)
        
        links_content_frame = ttk.Frame(links_frame)
        links_content_frame.pack(fill=tk.BOTH, expand=True)
        
        links_paned = ttk.PanedWindow(links_content_frame, orient=tk.HORIZONTAL)
        links_paned.pack(fill=tk.BOTH, expand=True)
        
        list_frame = ttk.Frame(links_paned)
        list_scroll_y = ttk.Scrollbar(list_frame, orient=tk.VERTICAL)
        list_scroll_x = ttk.Scrollbar(list_frame, orient=tk.HORIZONTAL)
        self._links_tree = ttk.Treeview(list_frame,
            columns=("type", "value", "context"),
            show="tree headings",
            yscrollcommand=list_scroll_y.set,
            xscrollcommand=list_scroll_x.set,
            height=12)
        list_scroll_y.config(command=self._links_tree.yview)
        list_scroll_x.config(command=self._links_tree.xview)
        
        self._links_tree.heading("#0", text="#")
        self._links_tree.heading("type", text="类型")
        self._links_tree.heading("value", text="链接/标识符")
        self._links_tree.heading("context", text="上下文")
        
        self._links_tree.column("#0", width=40, anchor="center")
        self._links_tree.column("type", width=80, anchor="center")
        self._links_tree.column("value", width=350)
        self._links_tree.column("context", width=200)
        
        self._links_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        list_scroll_y.pack(side=tk.RIGHT, fill=tk.Y)
        list_scroll_x.pack(side=tk.BOTTOM, fill=tk.X)
        
        self._links_tree.bind("<Double-Button-1>", self._on_link_double_click)
        
        detail_frame = ttk.LabelFrame(links_paned, text="详情", padding=5)
        self._link_detail_text = tk.Text(detail_frame, wrap=tk.WORD, font=("Microsoft YaHei UI", 9),
                                        bg="#fafafa", fg="#1f2937", relief=tk.FLAT)
        detail_scroll = ttk.Scrollbar(detail_frame, orient=tk.VERTICAL, command=self._link_detail_text.yview)
        self._link_detail_text.configure(yscrollcommand=detail_scroll.set)
        self._link_detail_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        detail_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        
        self._links_tree.bind("<<TreeviewSelect>>", self._on_link_selected)
        
        links_paned.add(list_frame, weight=3)
        links_paned.add(detail_frame, weight=1)
...
"""
    ...
    def _analyze_research_text(self):
        ...
        try:
            ...
            # 链接解析
            self._parse_and_display_links(text)
    ...
    def _parse_and_display_links(self, text: str):
        """解析文本中的链接、DOI、邮箱等"""
        import re
        from urllib.parse import urlparse
        ...
        self._link_detail_text.configure(state=tk.NORMAL)
        self._link_detail_text.delete(1.0, tk.END)
        self._link_detail_text.configure(state=tk.DISABLED)
        ...
        for link in found_links:
            key = link["full_value"].lower()[:100]
            if key not in seen:
                seen.add(key)
                unique_links.append(link)
        
        self._links_data = unique_links
        
        for i, link in enumerate(unique_links, 1):
            self._links_tree.insert("", tk.END, text=str(i), 
                                   values=(link["type"], link["value"], link["context"]))
        
        if unique_links:
            self._import_links_btn.configure(state=tk.NORMAL)
        else:
            self._import_links_btn.configure(state=tk.DISABLED)

    def _on_link_selected(self, event=None):
        """选中链接时显示详情"""
        selection = self._links_tree.selection()
        if not selection or not self._links_data:
            return
        
        idx = 0
        for i, item_id in enumerate(self._links_tree.get_children()):
            if item_id == selection[0]:
                idx = i
                break
        
        if idx < len(self._links_data):
            link = self._links_data[idx]
            self._link_detail_text.configure(state=tk.NORMAL)
            self._link_detail_text.delete(1.0, tk.END)
            self._link_detail_text.insert(tk.END, f"类型: {link['type']}\n\n", "label")
            self._link_detail_text.insert(tk.END, f"完整链接:\n{link['full_value']}\n\n", "value")
            self._link_detail_text.insert(tk.END, f"上下文:\n{link['context']}\n\n", "context")
            self._link_detail_text.insert(tk.END, "\n💡 双击可打开链接", "hint")
            self._link_detail_text.configure(state=tk.DISABLED)
            
            self._link_detail_text.tag_configure("label", foreground="#6b7280", font=("", 9, "bold"))
            self._link_detail_text.tag_configure("value", foreground="#3b82f6", font=("Consolas", 9))
            self._link_detail_text.tag_configure("context", foreground="#374151", font=("", 9))
            self._link_detail_text.tag_configure("hint", foreground="#9ca3af", font=("", 8))

    def _on_link_double_click(self, event=None):
        """双击打开链接"""
        selection = self._links_tree.selection()
        if not selection or not self._links_data:
            return
        
        region = self._links_tree.identify("region", event.x, event.y)
        if region != "cell":
            return
        ...
        if idx < len(self._links_data):
            ...
            if url.startswith("mailto:"):
                messagebox.showinfo("邮箱", f"邮箱地址: {url.replace('mailto:', '')}")
            else:
                try:
                    webbrowser.open(url)
                except Exception as e:
    ...
    def _import_links_to_hub(self):
        """导入链接到数据中心"""
        if not self._links_data:
            messagebox.showwarning("提示", "没有可导入的链接")
            return
        ...
        try:
            ...
            for link in self._links_data:
                ...
                is_doi = "doi" in link_type.lower() or "doi.org" in url
                is_paper = any(x in url.lower() for x in ["arxiv", "pubmed", "ncbi", "nature", "science", "ieee", "acm", "springer", "wiley"])
                
                if is_paper or is_doi:
                    item_type = ItemType.PAPER.value
                    tags = ["scholar", "paper", "auto_import", "from_analysis"]
                else:
                    item_type = ItemType.BOOKMARK.value
                    tags = ["bookmark", "url", "auto_import", "from_analysis"]
                
                hub_item = HubItem(
                    title=f"{link_type} - {url[:50]}",
                    content=f"从文本分析提取的链接\n\n上下文: {context}",
                    summary=link_type,
                    url=url,
            ...
            self._links_hub_imported = True
            self._import_links_btn.configure(state=tk.DISABLED)
            messagebox.showinfo("导入完成", f"已导入 {imported} 个链接到数据中心")
        except Exception as e:
    ...
    def _show_web_crawler(self):
        ...
        try:
            ...
            hub_bar = ttk.Frame(self.content_frame)
            hub_bar.pack(fill=tk.X, padx=5, pady=2)
            ttk.Button(hub_bar, text="📥 从数据枢纽导入URL", command=self._import_from_hub_crawler).pack(side=tk.LEFT, padx=5)
    ...
    def _switch_search_space(self):
        space = self._search_space_var.get()
        if space == "bookmarks":
            self._clear_search_right()
        elif space == "rss":
            self._show_rss_space()
        elif space == "hub":
            self._show_hub_space()
        elif space == "probe":
            self._show_probe_space()

    def _show_rss_space(self):
        for widget in self._search_right_frame.winfo_children():
            widget.destroy()
        container = ttk.Frame(self._search_right_frame, padding=10)
        container.pack(fill=tk.BOTH, expand=True)

        ttk.Label(container, text="📡 RSS 订阅管理", font=("", 14, "bold")).pack(anchor="w", pady=(0, 10))

        add_row = ttk.Frame(container)
        add_row.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(add_row, text="URL:").pack(side=tk.LEFT)
        self._rss_add_url_var = tk.StringVar()
        ttk.Entry(add_row, textvariable=self._rss_add_url_var, width=40).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ttk.Label(add_row, text="标题:").pack(side=tk.LEFT)
        self._rss_add_title_var = tk.StringVar()
        ttk.Entry(add_row, textvariable=self._rss_add_title_var, width=15).pack(side=tk.LEFT, padx=3)
        ttk.Button(add_row, text="➕ 订阅", command=self._add_rss_from_search, width=8).pack(side=tk.LEFT, padx=3)

        self._rss_tree_search = ttk.Treeview(container, columns=("title", "url", "status", "last"), show="headings", height=15)
        self._rss_tree_search.heading("title", text="标题")
        self._rss_tree_search.heading("url", text="URL")
        self._rss_tree_search.heading("status", text="状态")
        self._rss_tree_search.heading("last", text="上次轮询")
        self._rss_tree_search.column("title", width=150)
        self._rss_tree_search.column("url", width=300)
        self._rss_tree_search.column("status", width=50)
        self._rss_tree_search.column("last", width=130)
        rsb = ttk.Scrollbar(container, orient=tk.VERTICAL, command=self._rss_tree_search.yview)
        self._rss_tree_search.configure(yscrollcommand=rsb.set)
        self._rss_tree_search.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        rsb.pack(side=tk.RIGHT, fill=tk.Y)

        ctrl_row = ttk.Frame(container)
        ctrl_row.pack(fill=tk.X, pady=(5, 0))
        ttk.Button(ctrl_row, text="🔄 立即轮询全部", command=self._poll_rss_from_search, width=14).pack(side=tk.LEFT, padx=3)
        ttk.Button(ctrl_row, text="🔍 发现订阅源", command=self._discover_subscriptions_from_search, width=14).pack(side=tk.LEFT, padx=3)
        self._rss_search_status = ttk.Label(ctrl_row, text="", foreground="#666", font=("", 9))
        self._rss_search_status.pack(side=tk.LEFT, padx=10)

        self._refresh_rss_search_tree()

    def _refresh_rss_search_tree(self):
        if not hasattr(self, '_rss_tree_search') or not self._rss_tree_search.winfo_exists():
            return
        for row in self._rss_tree_search.get_children():
            self._rss_tree_search.delete(row)
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                return
            for sub in hub.get_rss_subscriptions():
                self._rss_tree_search.insert("", tk.END, values=(
                    sub.title or "未命名", sub.url[:60],
                    "✅" if sub.active else "⏸️",
                    sub.last_polled[:16] if sub.last_polled else "从未",
                ), tags=(sub.id,))
            self._rss_search_status.config(text=f"共 {len(hub.get_rss_subscriptions())} 个订阅源")
        except Exception:
            pass

    def _add_rss_from_search(self):
        url = self._rss_add_url_var.get().strip()
        if not url:
            return
        try:
            from ws2_data_hub import get_data_hub, RSSSubscription
            hub = get_data_hub()
            if hub:
                sub = RSSSubscription(url=url, title=self._rss_add_title_var.get().strip())
                hub.add_rss_subscription(sub)
                self._rss_add_url_var.set("")
                self._rss_add_title_var.set("")
                self._refresh_rss_search_tree()
        except Exception as e:
            messagebox.showerror("错误", f"添加RSS订阅失败: {e}")

    def _poll_rss_from_search(self):
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if hub:
                self._rss_search_status.config(text="轮询中...")
                import threading
                def poll():
                    results = hub.poll_all_rss_feeds()
                    total = sum(v for v in results.values() if v > 0)
                    self.root.after(0, lambda: self._rss_search_status.config(text=f"轮询完成: {total}条新内容"))
                    self.root.after(0, self._refresh_rss_search_tree)
                threading.Thread(target=poll, daemon=True).start()
        except Exception as e:
            self._rss_search_status.config(text=f"轮询失败: {e}")

    def _show_hub_space(self):
        for widget in self._search_right_frame.winfo_children():
            widget.destroy()
        container = ttk.Frame(self._search_right_frame, padding=10)
        container.pack(fill=tk.BOTH, expand=True)

        ttk.Label(container, text="🔗 数据枢纽 — 快速浏览", font=("", 14, "bold")).pack(anchor="w", pady=(0, 10))

        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                ttk.Label(container, text="数据枢纽未初始化", foreground="#999").pack(pady=20)
                return

            stats = hub.get_statistics()
            stats_row = ttk.Frame(container)
            stats_row.pack(fill=tk.X, pady=(0, 10))
            for label, val in [("📦 总项目", stats["total_items"]), ("⭐ 星标", stats["starred"]),
                               ("📩 未读", stats["unread"]), ("📡 RSS源", stats["rss_subscriptions"]),
                               ("📁 集合", stats["collections"])]:
                ttk.Label(stats_row, text=f"{label}: {val}", font=("", 10)).pack(side=tk.LEFT, padx=8)

            filter_row = ttk.Frame(container)
            filter_row.pack(fill=tk.X, pady=(0, 5))
            ttk.Label(filter_row, text="来源:").pack(side=tk.LEFT)
            self._hub_source_var = tk.StringVar(value="全部")
            for text, val in [("全部", "全部"), ("书签", "bookmark"), ("RSS", "rss"),
                              ("爬虫", "crawler"), ("Agent", "agent"), ("分析", "analysis")]:
                ttk.Radiobutton(filter_row, text=text, value=val,
                                variable=self._hub_source_var,
                                command=self._refresh_hub_items_tree).pack(side=tk.LEFT, padx=3)

            cols = ("icon", "title", "tags", "quality", "updated")
            self._hub_items_tree = ttk.Treeview(container, columns=cols, show="headings", height=18)
            self._hub_items_tree.heading("icon", text="")
            self._hub_items_tree.heading("title", text="标题")
            self._hub_items_tree.heading("tags", text="标签")
            self._hub_items_tree.heading("quality", text="质量")
            self._hub_items_tree.heading("updated", text="时间")
            self._hub_items_tree.column("icon", width=25, stretch=False)
            self._hub_items_tree.column("title", width=300)
            self._hub_items_tree.column("tags", width=100)
            self._hub_items_tree.column("quality", width=40, stretch=False)
            self._hub_items_tree.column("updated", width=110, stretch=False)
            hsb = ttk.Scrollbar(container, orient=tk.VERTICAL, command=self._hub_items_tree.yview)
            self._hub_items_tree.configure(yscrollcommand=hsb.set)
            self._hub_items_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
            hsb.pack(side=tk.RIGHT, fill=tk.Y)
            self._hub_items_tree.bind("<Double-1>", self._on_hub_item_double_click)

            self._refresh_hub_items_tree()
        except Exception as e:
            ttk.Label(container, text=f"加载失败: {e}", foreground="#e74c3c").pack(pady=20)

    def _refresh_hub_items_tree(self):
        if not hasattr(self, '_hub_items_tree') or not self._hub_items_tree.winfo_exists():
            return
        for row in self._hub_items_tree.get_children():
            self._hub_items_tree.delete(row)
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                return
            source = self._hub_source_var.get()
            items = hub.query_items(
                source_type=source if source != "全部" else None,
                limit=50, sort_by="updated_at", sort_desc=True,
            )
            icons = {"crawler": "🕷️", "rss": "📡", "agent": "🤖", "bookmark": "🔖",
                     "analysis": "🔬", "manual": "✏️", "local_file": "📂"}
            for item in items:
                icon = icons.get(item.source_type, "📦")
                score = getattr(item, 'quality_score', 0.0)
                self._hub_items_tree.insert("", tk.END, values=(
                    icon, item.title[:60], ", ".join(item.tags[:3]),
                    f"{score:.1f}", item.updated_at[:16],
                ), tags=(item.id,))
        except Exception:
            pass

    def _on_hub_item_double_click(self, event):
        sel = self._hub_items_tree.selection()
        if not sel:
            return
        item_id = self._hub_items_tree.item(sel[0], "tags")[0]
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if hub:
    ...
    def _show_probe_space(self):
        for widget in self._search_right_frame.winfo_children():
            widget.destroy()
        container = ttk.Frame(self._search_right_frame, padding=10)
        container.pack(fill=tk.BOTH, expand=True)

        ttk.Label(container, text="🔍 轻度探测 — 发现订阅源和元信息", font=("", 14, "bold")).pack(anchor="w", pady=(0, 10))

        input_row = ttk.Frame(container)
        input_row.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(input_row, text="URL:").pack(side=tk.LEFT)
        self._probe_url_var = tk.StringVar()
        ttk.Entry(input_row, textvariable=self._probe_url_var, width=50).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ttk.Button(input_row, text="🔍 探测", command=self._probe_single_url, width=8).pack(side=tk.LEFT, padx=3)

        batch_row = ttk.Frame(container)
        batch_row.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(batch_row, text="批量探测:", font=("", 10)).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(batch_row, text="📚 从书签探测", command=self._probe_from_bookmarks, width=14).pack(side=tk.LEFT, padx=3)
        ttk.Button(batch_row, text="🔗 从枢纽探测", command=self._discover_subscriptions_from_search, width=14).pack(side=tk.LEFT, padx=3)
        self._probe_status = ttk.Label(batch_row, text="", foreground="#666", font=("", 9))
        self._probe_status.pack(side=tk.LEFT, padx=10)

        self._probe_result_frame = ttk.Frame(container)
        self._probe_result_frame.pack(fill=tk.BOTH, expand=True)

    def _probe_single_url(self):
        url = self._probe_url_var.get().strip()
        if not url:
            return
        self._probe_status.config(text="探测中...")
        import threading
        ...
        def probe():
            try:
                ...
                if hub:
                    ...
                    self.root.after(0, lambda: self._show_probe_result(info))
            except Exception as e:
                self.root.after(0, lambda: self._probe_status.config(text=f"探测失败: {e}"))

        threading.Thread(target=probe, daemon=True).start()

    def _probe_from_bookmarks(self):
        self._probe_status.config(text="从书签批量探测中...")
        import threading
        ...
        def probe():
            try:
                ...
                hub = get_data_hub()
                if hub:
                    subs = hub.discover_subscriptions(source_type="bookmark", limit=20)
                    self.root.after(0, lambda: self._show_probe_batch_result(subs))
            except Exception as e:
                self.root.after(0, lambda: self._probe_status.config(text=f"探测失败: {e}"))

        threading.Thread(target=probe, daemon=True).start()

    def _show_probe_result(self, info):
        self._probe_status.config(text=f"探测完成: {info.get('title', '?')[:30]}")
        for w in self._probe_result_frame.winfo_children():
            w.destroy()

        meta = ttk.LabelFrame(self._probe_result_frame, text="元信息", padding=5)
        ...
        ttk.Label(meta, text="\n".join(lines), font=("Consolas", 10), justify=tk.LEFT).pack(anchor="w")

        feeds = info.get("feeds", [])
        if feeds:
            feed_frame = ttk.LabelFrame(self._probe_result_frame, text=f"📡 发现 {len(feeds)} 个订阅源", padding=5)
            feed_frame.pack(fill=tk.X, pady=5)
            for feed in feeds:
                row = ttk.Frame(feed_frame)
                row.pack(fill=tk.X, pady=1)
                ttk.Label(row, text=f"  {feed.get('type', 'rss').upper()}:", font=("", 9, "bold"), width=5).pack(side=tk.LEFT)
                ttk.Label(row, text=feed.get("title", "")[:30], width=30).pack(side=tk.LEFT, padx=3)
            ...
            def import_feeds():
                try:
                    ...
                    messagebox.showinfo("导入完成", f"已导入 {count} 个RSS订阅源")
                except Exception as e:
                    messagebox.showerror("错误", f"导入失败: {e}")

            ttk.Button(feed_frame, text=f"📥 全部导入为RSS订阅 ({len(feeds)})", command=import_feeds).pack(pady=5)

        links = info.get("links", [])
        if links:
            link_frame = ttk.LabelFrame(self._probe_result_frame, text=f"🔗 页面链接 ({len(links)})", padding=5)
            link_frame.pack(fill=tk.BOTH, expand=True, pady=5)
            link_text = tk.Text(link_frame, wrap=tk.WORD, font=("Consolas", 9), height=8)
            ...
            def add_links():
                try:
                    ...
                    added = 0
                    for link in links:
                        item = HubItem(title=link.split("/")[-1] or link, url=link,
                                       source_type=SourceType.BOOKMARK.value,
                                       item_type=ItemType.BOOKMARK.value, pipeline_stage="ingested")
                        try:
                            hub.add_item(item)
                            added += 1
                        except Exception:
                            pass
                    messagebox.showinfo("导入完成", f"已导入 {added} 个链接到数据枢纽")
                except Exception as e:
                    messagebox.showerror("错误", f"导入失败: {e}")

            ttk.Button(link_frame, text=f"📥 导入全部链接到枢纽 ({len(links)})", command=add_links).pack(pady=3)

    def _show_probe_batch_result(self, subscriptions):
        if not subscriptions:
            self._probe_status.config(text="未发现新的订阅源")
            return
        self._probe_status.config(text=f"发现 {len(subscriptions)} 个订阅源")
        for w in self._probe_result_frame.winfo_children():
            w.destroy()

        list_frame = ttk.LabelFrame(self._probe_result_frame,
                                     text=f"📡 发现 {len(subscriptions)} 个RSS/Atom订阅源", padding=5)
        ...
        tree.column("source", width=150)
        tsb = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=tree.yview)
        ...
        def import_all():
            try:
                from ws2_data_hub import get_data_hub
                hub = get_data_hub()
                count = hub.import_discovered_subscriptions(subscriptions)
                messagebox.showinfo("导入完成", f"已导入 {count} 个RSS订阅源")
            except Exception as e:
                messagebox.showerror("错误", f"导入失败: {e}")

        ttk.Button(btn_row, text=f"📥 全部导入 ({len(subscriptions)})", command=import_all).pack(side=tk.LEFT, padx=5)

    def _discover_subscriptions_from_search(self):
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                messagebox.showinfo("提示", "数据枢纽未初始化")
                return
            import threading

            def discover():
                subs = hub.discover_subscriptions(source_type="bookmark", limit=30)
                self.root.after(0, lambda: self._show_discover_dialog(subs))

            threading.Thread(target=discover, daemon=True).start()
        except Exception as e:
            messagebox.showerror("错误", f"发现订阅源失败: {e}")

    def _show_discover_dialog(self, subscriptions):
        if not subscriptions:
            messagebox.showinfo("提示", "未发现新的RSS/Atom订阅源")
            return
        dialog = tk.Toplevel(self.root)
    ...
    def _import_browser_to_hub(self):
        try:
            from ws2_data_hub import get_data_hub, HubItem, SourceType, ItemType
            hub = get_data_hub()
            if not hub:
            ...
            messagebox.showerror("错误", "数据枢纽模块不可用")
            return
        ...
        if not file_path:
            ...
            for p in possible:
                if os.path.exists(p):
                    ...
                    break
            if not file_path:
                messagebox.showinfo("提示", "未找到浏览器书签文件，请手动选择")
                return
            ...
            def extract(nodes):
                for node in nodes:
                    if node.get("type") == "url":
                        ...
                        extract(node.get("children", []))
            roots = data.get("roots", {})
            for key in ["bookmark_bar", "other_bookmarks", "synced"]:
                if key in roots:
                    extract(roots[key].get("children", []))
            if imported:
                self._select_and_import_to_hub(imported, hub)
            else:
                messagebox.showinfo("提示", "未找到可导入的书签")
        except Exception as e:
            messagebox.showerror("错误", f"导入失败: {e}")

    def _select_and_import_to_hub(self, imported_bookmarks, hub):
        dlg = tk.Toplevel(self.root)
        dlg.title(f"导入浏览器书签到数据枢纽 ({len(imported_bookmarks)}个)")
        dlg.geometry("750x550")
        dlg.transient(self.root)
        dlg.grab_set()
        ...
        ttk.Label(search_row, text="🔍 搜索:").pack(side=tk.LEFT)
        filter_var = tk.StringVar()
        filter_entry = ttk.Entry(search_row, textvariable=filter_var, width=40)
        ...
        all_items = []
        for i, bm in enumerate(imported_bookmarks):
            iid = tree.insert("", tk.END, values=(bm.get("name", ""), bm.get("url", ""), bm.get("category", "")), tags=(i,))
            all_items.append(iid)
        tree.selection_set(all_items)
        ...
        def do_import():
            ...
            for item in selected:
                ...
                try:
                    ...
                    pass
            messagebox.showinfo("导入完成", f"已导入 {count} 个书签到数据枢纽")
            dlg.destroy()

        btn_frame = ttk.Frame(dlg)
        btn_frame.pack(pady=10)
        ttk.Button(btn_frame, text="📥 导入选中到枢纽", command=do_import).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="取消", command=dlg.destroy).pack(side=tk.LEFT, padx=5)

    def _import_from_hub_bookmarks(self):
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                messagebox.showinfo("提示", "数据枢纽未初始化")
                return
            items = hub.query_items(source_type="bookmark", limit=200, sort_by="updated_at", sort_desc=True)
            if not items:
                messagebox.showinfo("提示", "数据枢纽中没有书签数据")
                return
            count = 0
            for item in items:
                if not item.url:
                ...
                for bm in self._bookmarks:
                    if bm.get("url") == item.url:
                        exists = True
                        break
                    for child in bm.get("children", []):
                        if child.get("url") == item.url:
                            exists = True
                            break
                if not exists:
                    new_bm = {
                        "name": item.title, "url": item.url,
                        "category": item.tags[0] if item.tags else "tool",
                        "icon": "🔖", "color": "#3498db",
                        "id": f"hub_{item.id[:8]}", "children": [],
                    }
                    if item.summary:
                        new_bm["description"] = item.summary[:200]
                    self._bookmarks.append(new_bm)
                    count += 1
            if count > 0:
                self._save_bookmarks()
                self._refresh_bookmarks()
                messagebox.showinfo("导入完成", f"从数据枢纽导入了 {count} 个书签")
            else:
                messagebox.showinfo("提示", "没有新的书签需要导入（全部已存在）")
        except Exception as e:
            messagebox.showerror("错误", f"从数据枢纽导入失败: {e}")

    def _import_from_hub_crawler(self):
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                messagebox.showinfo("提示", "数据枢纽未初始化")
                return
            items = hub.query_items(source_type="bookmark", limit=100, sort_by="quality_score", sort_desc=True)
            urls = [item.url for item in items if item.url and not item.content]
            if not urls:
                items2 = hub.query_items(limit=50, sort_by="quality_score", sort_desc=True)
                urls = [item.url for item in items2 if item.url]
            if not urls:
                messagebox.showinfo("提示", "数据枢纽中没有可爬取的URL")
                return
            if hasattr(self, '_web_crawler_ui') and self._web_crawler_ui:
                for url in urls[:20]:
                    try:
                        if hasattr(self._web_crawler_ui, 'add_url'):
                            self._web_crawler_ui.add_url(url)
                    except Exception:
                        pass
                messagebox.showinfo("导入完成", f"已将 {min(len(urls), 20)} 个URL导入爬虫")
            else:
                url_list = "\n".join(urls[:20])
                messagebox.showinfo("数据枢纽URL", f"以下URL可导入爬虫:\n\n{url_list}")
        except Exception as e:
            messagebox.showerror("错误", f"从数据枢纽导入失败: {e}")

    def _import_from_hub_analysis(self):
        try:
            from ws2_data_hub import get_data_hub
            hub = get_data_hub()
            if not hub:
                messagebox.showinfo("提示", "数据枢纽未初始化")
                return
            items = hub.query_items(limit=50, sort_by="quality_score", sort_desc=True)
            items_with_content = [i for i in items if i.content and len(i.content) > 100]
            if not items_with_content:
                messagebox.showinfo("提示", "数据枢纽中没有足够内容的数据项")
                return
            dialog = tk.Toplevel(self.root)
            dialog.title("从数据枢纽导入文本")
            dialog.geometry("500x400")
            dialog.transient(self.root)
            dialog.grab_set()
            ttk.Label(dialog, text=f"找到 {len(items_with_content)} 个含内容的数据项，选择要导入的:").pack(anchor="w", padx=15, pady=10)
            list_frame = ttk.Frame(dialog)
            list_frame.pack(fill=tk.BOTH, expand=True, padx=15)
            cols = ("title", "source", "len")
            tree = ttk.Treeview(list_frame, columns=cols, show="headings", height=12)
            tree.heading("title", text="标题")
            tree.heading("source", text="来源")
            tree.heading("len", text="字符数")
            tree.column("title", width=250)
            tree.column("source", width=80)
            tree.column("len", width=70)
            sb = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=tree.yview)
            tree.configure(yscrollcommand=sb.set)
            tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
            sb.pack(side=tk.RIGHT, fill=tk.Y)
            for item in items_with_content:
                tree.insert("", tk.END, values=(item.title[:50], item.source_type, len(item.content)), tags=(item.id,))

            def do_import():
                sel = tree.selection()
                if not sel:
                    return
                selected_ids = [tree.item(s, "tags")[0] for s in sel]
                texts = []
                for sid in selected_ids:
                    hub_item = hub.get_item(sid)
                    if hub_item and hub_item.content:
                        texts.append(f"=== {hub_item.title} ===\n{hub_item.content[:3000]}")
                if texts and hasattr(self, '_research_text') and self._research_text:
                    combined = "\n\n".join(texts)
                    self._research_text.delete(1.0, tk.END)
                    self._research_text.insert(1.0, combined)
                dialog.destroy()
    ...
    def _show_synergy_hub(self):
        ...
        self.nav_title.config(text="🔗 数据枢纽")
        self._update_overall_label()
        ...
        try:
            from ws2_data_hub_ui import DataHubUI
            self._data_hub_ui = DataHubUI(main_container, BASE, self)
            self._data_hub_ui.frame.pack(fill=tk.BOTH, expand=True)
        except Exception as e:
            msg = ttk.Label(main_container,
                          text=f"⚠️ 数据枢纽加载失败\n\n"
                               f"错误: {e}\n\n"
                               "数据枢纽功能：\n"
                               "• 📊 总览仪表盘 — 统计卡片、来源分布\n"
                               ...
                               "• 🔧 数据管道 — 爬虫/书签/分析/本地导入\n"
                               "• 📈 统计分析\n\n"
                               "请确保 ws2_data_hub.py 和 ws2_data_hub_ui.py 存在",
                          justify=tk.CENTER)
            msg.pack(pady=20)
            ...
            def try_load_hub():
                try:
                    ...
                    messagebox.showerror("错误", f"加载数据枢纽失败: {e2}")
    ...
    def _show_course_simulation_page(self):
        ...
        try:
            from mcp.automation.course_simulation import (
                CourseSimulationEngine, Timetable, TimetableSlot,
                DAY_NAMES, PERIODS, COURSE_COLORS
            )
            has_sim = True
        except Exception:
            has_sim = False
        ...
        if not has_sim:
            ttk.Label(self.content_frame,
                      text="⚠️ 课程模拟模块未加载",
                      foreground="red").pack(padx=15, pady=20)
            return

        if not hasattr(self, '_course_sim_engine'):
            self._course_sim_engine = CourseSimulationEngine(root=self.root)

        sim = self._course_sim_engine
        
        # 获取当前周数和日期范围
        week_num = sim.get_week_number()
        date_range = sim.get_date_range_for_week()
        current_period_idx = sim.get_current_period_index()
        ...
        ttk.Label(title_frame, text="🎓 课程系统",
                  font=("Microsoft YaHei UI", 14, "bold")).pack(anchor=tk.W)
        week_info_label = ttk.Label(title_frame, 
        ...
        self._course_monitor_var = tk.StringVar(value="🟢 空闲")
        ttk.Label(status_frame, textvariable=self._course_monitor_var,
                  font=("Microsoft YaHei UI", 11)).pack(anchor=tk.E)
        self._course_countdown_var = tk.StringVar(value="")
        ttk.Label(status_frame, textvariable=self._course_countdown_var,
                  font=("Microsoft YaHei UI", 10), foreground="#3b82f6").pack(anchor=tk.E)

        week_frame = ttk.Frame(self.content_frame)
        week_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=4)

        canvas = tk.Canvas(week_frame, bg="#f1f5f9", highlightthickness=0)
        v_scroll = ttk.Scrollbar(week_frame, orient=tk.VERTICAL, command=canvas.yview)
        h_scroll = ttk.Scrollbar(week_frame, orient=tk.HORIZONTAL, command=canvas.xview)
        ...
        # 表头容器（使用grid保证对齐）
        header_container = tk.Frame(inner, bg="#e2e8f0")
        header_container.pack(fill=tk.X)

        # 角落格子
        corner_frame = tk.Frame(header_container, width=CORNER_WIDTH, height=HEADER_HEIGHT, bg="#1e293b")
        corner_frame.pack_propagate(False)
        corner_frame.pack(side=tk.LEFT)
        tk.Label(corner_frame, text="节次\\星期", font=("Microsoft YaHei UI", 9, "bold"),
                 bg="#1e293b", fg="#f1f5f9", anchor=tk.CENTER).pack(fill=tk.BOTH, expand=True)

        current_dow = datetime.now().isoweekday()
        for day in range(1, 8):
            is_today = (day == current_dow)
            date_str = sim.get_date_for_weekday(day)
            
            bg = "#0f172a" if is_today else "#1e293b"
            fg = "#fbbf24" if is_today else "#e2e8f0"
            
            day_header = tk.Frame(header_container, width=CELL_WIDTH, height=HEADER_HEIGHT, bg=bg)
            day_header.pack_propagate(False)
            day_header.pack(side=tk.LEFT)
            
            tk.Label(day_header, text=DAY_NAMES.get(day, ""), font=("Microsoft YaHei UI", 10, "bold"),
                     bg=bg, fg=fg, anchor=tk.CENTER).pack(fill=tk.BOTH, expand=True, pady=(4, 0))
            tk.Label(day_header, text=date_str, font=("Microsoft YaHei UI", 8),
                     bg=bg, fg=fg, anchor=tk.CENTER).pack(fill=tk.X, pady=(0, 4))

        week_slots = sim.get_week_slots()
        current_slot = sim.get_current_slot()
        
        # 存储需要动画的组件
        animated_widgets = []

        for p_idx, (period_name, p_start, p_end) in enumerate(PERIODS):
            row_container = tk.Frame(inner, bg="#cbd5e1")
            row_container.pack(fill=tk.X)
            
            is_current_period = (p_idx == current_period_idx)

            # 时间列 - 根据时段类型设置不同的颜色
            period_name_text = period_name
            ...
            time_col = tk.Frame(row_container, width=TIME_COL_WIDTH, height=CELL_HEIGHT, bg=time_bg)
            time_col.pack_propagate(False)
            time_col.pack(side=tk.LEFT)
            
            time_text = f"{period_icon}\n{period_name_text}\n{p_start}"
            tk.Label(time_col, text=time_text, font=("Microsoft YaHei UI", 7),
                     bg=time_bg, fg=time_fg,
                     anchor=tk.CENTER, justify=tk.CENTER).pack(fill=tk.BOTH, expand=True)

            for day in range(1, 8):
                key = f"{day}_{p_idx}"
                slot = week_slots.get(key)
                is_today = (day == current_dow)
                is_current = (current_slot is not None and slot is not None
                              and slot.slot_id == current_slot.slot_id)
                is_current_period_cell = (is_current_period and is_today)
                ...
                if is_current:
                    ...
                    bg = slot.color if slot.color else "#3b82f6"
                    fg = "#ffffff"
                    bd_color = bg
                    bd_width = 1
                elif is_current_period_cell:
                    bg = "#fef3c7"
                    fg = "#fbbf24"
                    bd_color = "#fbbf24"
                    bd_width = 2
                ...
                cell = tk.Frame(row_container, width=CELL_WIDTH, height=CELL_HEIGHT,
                                bg=bg, highlightbackground=bd_color, highlightcolor=bd_color,
                                highlightthickness=bd_width, cursor="hand2")
                cell.pack_propagate(False)
                cell.pack(side=tk.LEFT)

                if slot:
                    # 获取当前周数
                    current_week = sim.get_week_number()
                    
                    # 课程名称
                    course_label = tk.Label(cell, text=slot.course_name,
                                            font=("Microsoft YaHei UI", 9, "bold"),
                                            bg=bg, fg=fg, anchor=tk.CENTER, wraplength=CELL_WIDTH-8)
                    course_label.pack(fill=tk.X, padx=4, pady=(4, 2))

                    # 时间信息
                    time_text = f"⏰ {slot.start_time}"
                    time_cell_label = tk.Label(cell, text=time_text,
                                              font=("Microsoft YaHei UI", 7),
                                              bg=bg, fg=fg, anchor=tk.CENTER)
                    time_cell_label.pack(fill=tk.X, padx=4, pady=0)

                    # 详细信息
                    detail_text = ""
                    if slot.location:
                        detail_text = f"📍{slot.location}"
                    if slot.teacher:
                        if detail_text:
                            detail_text += f" 👨‍🏫{slot.teacher}"
                        else:
                            detail_text = f"👨‍🏫{slot.teacher}"
                    
                    if detail_text:
                        detail_label = tk.Label(cell, text=detail_text,
                                                font=("Microsoft YaHei UI", 7),
                                                bg=bg, fg=fg, anchor=tk.CENTER, wraplength=CELL_WIDTH-8)
                        detail_label.pack(fill=tk.X, padx=4, pady=(2, 2))

                    for widget in [cell, course_label, time_cell_label] + ([detail_label] if detail_text else []):
                        widget.bind("<Button-1>", lambda e, s=slot: self._on_course_cell_click(s))
                        widget.bind("<Button-3>", lambda e, s=slot: self._on_course_cell_right_click(s))

                    if is_current:
                        pulse_label = tk.Label(cell, text="🔴 进行中",
                                               font=("Microsoft YaHei UI", 7, "bold"),
                                               bg=bg, fg="#dc2626")
                        pulse_label.pack(fill=tk.X, padx=4, pady=(0, 4))
                        pulse_label.bind("<Button-1>", lambda e, s=slot: self._on_course_cell_click(s))
                        animated_widgets.append((pulse_label, bg))
                else:
                    # 根据时段类型设置空闲单元格的样式
                    is_break = False
                    break_text = ""
                    ...
                    if is_break:
                        # 显示休息/活动时段
                        break_label = tk.Label(cell, text=break_text,
                                              font=("Microsoft YaHei UI", 8, "bold"),
                                              bg=break_bg, fg=break_fg, anchor=tk.CENTER)
                        break_label.pack(fill=tk.BOTH, expand=True)
                    else:
                        # 普通空闲时段
                        empty_label = tk.Label(cell, text="+",
                                               font=("Microsoft YaHei UI", 16), bg=bg, fg=fg,
                                               anchor=tk.CENTER)
                        empty_label.pack(fill=tk.BOTH, expand=True)
                        empty_label.bind("<Button-1>", lambda e, d=day, p=p_idx: self._on_empty_cell_click(d, p))
                        cell.bind("<Button-1>", lambda e, d=day, p=p_idx: self._on_empty_cell_click(d, p))
        ...
        ttk.Button(ctrl_frame, text="⏹️ 退出课程模式",
                   command=self._exit_course_mode).pack(side=tk.LEFT, padx=3)
        
        # 添加动画效果
        if animated_widgets:
            self._start_pulse_animation(animated_widgets)

        self._update_course_monitor()
    
    def _start_pulse_animation(self, widgets):
        """开始脉冲动画效果"""
        self._pulse_state = 0
        self._pulse_widgets = widgets
        
        def animate():
            if not hasattr(self, '_pulse_widgets') or not self._pulse_widgets:
                return
                
            self._pulse_state = (self._pulse_state + 1) % 4
            alpha = 0.6 + 0.4 * abs(self._pulse_state - 2) / 2
            
            for label, bg in self._pulse_widgets:
                try:
                    if self._pulse_state % 2 == 0:
                        label.config(fg="#dc2626", font=("Microsoft YaHei UI", 7, "bold"))
                    else:
                        label.config(fg="#ef4444", font=("Microsoft YaHei UI", 8, "bold"))
                except:
                    pass
    ...
    def _on_course_cell_click(self, slot):
        from mcp.automation.course_simulation import DAY_NAMES
        course_info = self._get_course_detail(slot.course_id)
        
        # 获取周数信息
        sim = self._course_sim_engine
        current_week = sim.get_week_number()
        week_date = sim.get_date_for_weekday(slot.day_of_week)
        date_range = sim.get_date_range_for_week()

        dlg = tk.Toplevel(self.root)
        dlg.title(slot.course_name)
        ...
        ttk.Label(frame, text=f"📚 {slot.course_name}",
                  font=("Microsoft YaHei UI", 13, "bold")).pack(anchor=tk.W, pady=(0, 8))
        
        # 课时详细信息
        info_frame = ttk.LabelFrame(frame, text="课时详情", padding=10)
        info_frame.pack(fill=tk.X, pady=(0, 8))
        
        info_lines = [
            f"📅 第 {current_week} 周",
            f"📆 {DAY_NAMES.get(slot.day_of_week, '')} ({week_date})",
            f"⏰ 时间：{slot.start_time} - {slot.end_time}",
            f"📍 地点：{slot.location or '未指定'}",
            f"👨‍🏫 教师：{slot.teacher or '未指定'}"
        ]
        
        for line in info_lines:
            ttk.Label(info_frame, text=line, font=("Microsoft YaHei UI", 10),
                      justify=tk.LEFT).pack(anchor=tk.W, pady=2)

        if course_info:
            detail_frame = ttk.LabelFrame(frame, text="课程数据", padding=8)
            detail_frame.pack(fill=tk.X, pady=(0, 8))

            domain = course_info.get("domain", "")
            total_hours = course_info.get("total_hours", 0)
            sections = course_info.get("sections", [])
            lessons = course_info.get("lessons", [])
            completed = set(self.system.get_course_progress(slot.course_id).get("completed_lessons", []))
            pct = self.system.get_completion_pct(slot.course_id)

            for label, value in [
                ("领域", domain or "未分类"),
                ("总课时", f"{len(lessons)} 节"),
                ("总学时", f"{total_hours} 小时"),
                ("章节数", str(len(sections))),
                ("进度", f"{len(completed)}/{len(lessons)} ({pct}%)"),
            ]:
                row = ttk.Frame(detail_frame)
                row.pack(fill=tk.X, pady=1)
                ttk.Label(row, text=f"{label}:", width=8, anchor=tk.E,
                          font=("", 9)).pack(side=tk.LEFT, padx=(0, 5))
                ttk.Label(row, text=value, font=("", 9, "bold")).pack(side=tk.LEFT)

            if pct < 100:
                pbar = ttk.Progressbar(detail_frame, length=300, mode="determinate", maximum=100)
                pbar["value"] = pct
                pbar.pack(fill=tk.X, pady=(5, 0))

        btn_frame = ttk.Frame(frame)
        btn_frame.pack(fill=tk.X, side=tk.BOTTOM, pady=(8, 0))

        ttk.Button(btn_frame, text="▶️ 进入执行模式",
                   command=lambda: [dlg.destroy(), self._enter_course_execution(slot)]).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_frame, text="🎓 进入课程模式",
                   command=lambda: [dlg.destroy(), self._enter_specific_course(slot)]).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_frame, text="🗑️ 删除",
                   command=lambda: [self._delete_course_slot(slot), dlg.destroy()]).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_frame, text="关闭",
                   command=dlg.destroy).pack(side=tk.RIGHT, padx=3)

    def _get_course_detail(self, course_id: str):
        if not hasattr(self, 'system') or not self.system.courses:
            return None
        for c in self.system.courses:
            cid = c.get("note_id", "") if isinstance(c, dict) else getattr(c, 'note_id', '')
            if cid == course_id:
                return c
            title = c.get("course_title", "") if isinstance(c, dict) else getattr(c, 'name', '')
            if title == course_id:
                return c
        return None
    ...
    def _on_course_cell_right_click(self, slot):
        menu = tk.Menu(self.root, tearoff=0)
        menu.add_command(label=f"📚 {slot.course_name}", state=tk.DISABLED)
        menu.add_separator()
        menu.add_command(label="▶️ 进入此课程", command=lambda: self._enter_specific_course(slot))
        menu.add_command(label="🗑️ 删除此课", command=lambda: self._delete_course_slot(slot))
        try:
            menu.tk_popup(self.root.winfo_pointerx(), self.root.winfo_pointery())
        finally:
            menu.grab_release()

    def _on_empty_cell_click(self, day, period_idx):
        from mcp.automation.course_simulation import DAY_NAMES, PERIODS
        period_info = PERIODS[period_idx] if period_idx < len(PERIODS) else None

        dlg = tk.Toplevel(self.root)
        dlg.title(f"添加课程 - {DAY_NAMES.get(day, '')} 第{period_idx + 1}节")
        ...
        ttk.Label(frame, text=f"📅 {DAY_NAMES.get(day, '')} 第{period_idx + 1}节",
                  font=("Microsoft YaHei UI", 11, "bold")).pack(anchor=tk.W, pady=(0, 10))

        fields = ttk.Frame(frame)
        fields.pack(fill=tk.X)

        ttk.Label(fields, text="课程名称:").grid(row=0, column=0, sticky=tk.W, pady=3)
        name_var = tk.StringVar()
        course_options = []
        if hasattr(self, 'system') and self.system.courses:
            for c in self.system.courses:
                title = c.get("course_title", "") if isinstance(c, dict) else getattr(c, 'name', '')
                cid = c.get("note_id", title) if isinstance(c, dict) else getattr(c, 'note_id', title)
                domain = c.get("domain", "") if isinstance(c, dict) else getattr(c, 'domain', '')
                label = f"{title} [{domain}]" if domain else title
                course_options.append((label, cid, title))
        name_combo = ttk.Combobox(fields, textvariable=name_var,
                                   values=[opt[0] for opt in course_options], width=28)
        name_combo.grid(row=0, column=1, pady=3, padx=5)

        selected_course_id = tk.StringVar()

        def _on_course_selected(event):
            for label, cid, title in course_options:
                if name_var.get() == label:
                    selected_course_id.set(cid)
                    break

        name_combo.bind("<<ComboboxSelected>>", _on_course_selected)

        ttk.Label(fields, text="地点:").grid(row=1, column=0, sticky=tk.W, pady=3)
        loc_var = tk.StringVar()
        ttk.Entry(fields, textvariable=loc_var, width=22).grid(row=1, column=1, pady=3, padx=5)

        ttk.Label(fields, text="教师:").grid(row=2, column=0, sticky=tk.W, pady=3)
        teacher_var = tk.StringVar()
        ttk.Entry(fields, textvariable=teacher_var, width=22).grid(row=2, column=1, pady=3, padx=5)

        if period_info:
            ttk.Label(fields, text=f"时间: {period_info[1]}-{period_info[2]}",
                      foreground="#666").grid(row=3, column=0, columnspan=2, sticky=tk.W, pady=3)

        def do_add():
            if not name_var.get().strip():
                messagebox.showwarning("提示", "请选择或输入课程名称", parent=dlg)
                return
            from mcp.automation.course_simulation import TimetableSlot, PERIODS
            p = PERIODS[period_idx] if period_idx < len(PERIODS) else (f"第{period_idx+1}节", "08:00", "08:45")
            cid = selected_course_id.get() or name_var.get().strip()
            ...
            if tt:
                self._course_sim_engine.add_slot(tt.timetable_id, new_slot)
    ...
    def _enter_specific_course(self, slot):
        if not hasattr(self, '_course_sim_engine'):
            return
        self._course_sim_engine.enter_course_mode(slot)
        course = self._get_course_detail(slot.course_id)
        if course:
            real_id = course.get("note_id", slot.course_id) if isinstance(course, dict) else getattr(course, 'note_id', slot.course_id)
            self._show_execution_mode(real_id)
        else:
            self._show_course_simulation_page()

    def _delete_course_slot(self, slot):
        if not hasattr(self, '_course_sim_engine'):
            return
        if messagebox.askyesno("确认", f"确定删除课程 {slot.course_name} 吗？"):
            tt = self._course_sim_engine.get_active_timetable()
            if tt:
                self._course_sim_engine.remove_slot(tt.timetable_id, slot.slot_id)
            self._show_course_simulation_page()

    def _sync_courses_to_timetable(self):
        if not hasattr(self, '_course_sim_engine'):
            return
        if not hasattr(self, 'system') or not self.system.courses:
            messagebox.showinfo("提示", "课程库为空，请先导入课程数据")
            return

        from mcp.automation.course_simulation import TimetableSlot, COURSE_COLORS, DAY_NAMES, PERIODS

        tt = self._course_sim_engine.get_active_timetable()
        if not tt:
            return

        existing_ids = {s.course_id for s in tt.slots}
        ...
        ttk.Label(frame, text="选择要同步到课表的课程",
                  font=("Microsoft YaHei UI", 11, "bold")).pack(anchor=tk.W, pady=(0, 5))

        tree_frame = ttk.Frame(frame)
        tree_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        cols = ("选择", "课程名称", "领域", "周课时", "状态")
        tree = ttk.Treeview(tree_frame, columns=cols, show="headings", selectmode="extended")

        tree.heading("选择", text="选择")
        tree.heading("课程名称", text="课程名称")
        tree.heading("领域", text="领域")
        tree.heading("周课时", text="周课时")
        tree.heading("状态", text="状态")
        
        tree.column("选择", width=50, anchor="center")
        tree.column("课程名称", width=200)
        tree.column("领域", width=100)
        tree.column("周课时", width=60, anchor="center")
        ...
        course_data = {}
        for c in self.system.courses:
            title = c.get("course_title", "未命名") if isinstance(c, dict) else getattr(c, 'name', '未命名')
            cid = c.get("note_id", title) if isinstance(c, dict) else getattr(c, 'note_id', title)
            domain = c.get("domain", "") if isinstance(c, dict) else getattr(c, 'domain', "")
            weekly_hours = c.get("weekly_hours", 2) if isinstance(c, dict) else getattr(c, 'weekly_hours', 2)
            location = c.get("location", "") if isinstance(c, dict) else getattr(c, 'location', "")
            teacher = c.get("teacher", "") if isinstance(c, dict) else getattr(c, 'teacher', "")
            already = cid in existing_ids
            
            var = tk.BooleanVar(value=False)
            check_vars[cid] = var
            course_data[cid] = {"location": location, "teacher": teacher, "weekly_hours": weekly_hours}
            
            status = "✅ 已同步" if already else "⏳ 未同步"
            tree.insert("", tk.END, values=("☐", title, domain if domain else "-", f"{weekly_hours}节/周", status), tags=(cid,))
        ...
        def toggle_selection(event):
            ...
            if region == "cell":
                item_id = tree.identify_row(event.y)
                if item_id:
                    cid = tree.item(item_id, "tags")[0] if tree.item(item_id, "tags") else None
                    if cid and cid in check_vars:
                        current = check_vars[cid].get()
                        check_vars[cid].set(not current)
                        new_state = "☑" if not current else "☐"
                        values = list(tree.item(item_id, "values"))
                        values[0] = new_state
                        tree.item(item_id, values=values)

        tree.bind("<Double-Button-1>", toggle_selection)

        def select_all():
            for cid in check_vars:
                check_vars[cid].set(True)
        ...
        ttk.Button(select_frame, text="☑ 全选", command=select_all).pack(side=tk.LEFT, padx=2)
        ttk.Button(select_frame, text="☐ 取消全选", command=deselect_all).pack(side=tk.LEFT, padx=2)
        ttk.Button(select_frame, text="🔄 刷新", command=lambda: self._sync_courses_to_timetable()).pack(side=tk.LEFT, padx=2)

        settings_frame = ttk.LabelFrame(frame, text="排课设置", padding=8)
        settings_frame.pack(fill=tk.X, pady=(5, 3))

        row1 = ttk.Frame(settings_frame)
        row1.pack(fill=tk.X, pady=2)
        
        ttk.Label(row1, text="星期:").pack(side=tk.LEFT, padx=(0, 5))
        day_vars = {}
        for d in range(1, 8):
            day_vars[d] = tk.BooleanVar(value=False)
            cb = ttk.Checkbutton(row1, text=DAY_NAMES.get(d, "")[:2], variable=day_vars[d])
            cb.pack(side=tk.LEFT, padx=2)

        row2 = ttk.Frame(settings_frame)
        row2.pack(fill=tk.X, pady=2)
        
        ttk.Label(row2, text="节次:").pack(side=tk.LEFT, padx=(0, 5))
        period_vars = {}
        for p_idx in range(len(PERIODS)):
            period_vars[p_idx] = tk.BooleanVar(value=False)
            period_name = f"{p_idx+1}"
            period_time = f"{PERIODS[p_idx][1]}"
            cb = ttk.Checkbutton(row2, text=f"{period_name}({period_time[:5]})", variable=period_vars[p_idx])
            cb.pack(side=tk.LEFT, padx=1)
        
        ttk.Label(row2, text="自定义节数:").pack(side=tk.LEFT, padx=(20, 5))
        custom_hours_var = tk.IntVar(value=2)
        custom_hours_spin = ttk.Spinbox(row2, from_=1, to=10, textvariable=custom_hours_var, width=5)
        custom_hours_spin.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row2, text="地点:").pack(side=tk.LEFT, padx=(10, 5))
        location_entry = ttk.Entry(row2, width=15)
        location_entry.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row2, text="教师:").pack(side=tk.LEFT, padx=(10, 5))
        teacher_entry = ttk.Entry(row2, width=15)
        teacher_entry.pack(side=tk.LEFT, padx=5)

        row3 = ttk.Frame(settings_frame)
        row3.pack(fill=tk.X, pady=2)
        
        ttk.Label(row3, text="学期开始:").pack(side=tk.LEFT, padx=(0, 5))
        start_date_entry = ttk.Entry(row3, width=12)
        start_date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))
        start_date_entry.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row3, text="学期结束:").pack(side=tk.LEFT, padx=(10, 5))
        end_date_entry = ttk.Entry(row3, width=12)
        end_date_entry.insert(0, (datetime.now().replace(month=7 if datetime.now().month < 7 else 1) + 
                                  (timedelta(days=120) if datetime.now().month < 7 else timedelta(days=0))).strftime("%Y-%m-%d"))
        end_date_entry.pack(side=tk.LEFT, padx=5)

        manage_frame = ttk.LabelFrame(frame, text="已同步课程管理", padding=8)
        manage_frame.pack(fill=tk.X, pady=(5, 3))

        manage_row = ttk.Frame(manage_frame)
        manage_row.pack(fill=tk.X)
        
        ttk.Label(manage_row, text="已同步:").pack(side=tk.LEFT, padx=(0, 5))
        existing_tree_frame = ttk.Frame(manage_row)
        existing_tree_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        
        existing_tree = ttk.Treeview(existing_tree_frame, columns=("课程", "时间", "地点"), 
                                      show="headings", height=4)
        existing_tree.heading("课程", text="课程")
        existing_tree.heading("时间", text="时间")
        existing_tree.heading("地点", text="地点")
        existing_tree.column("课程", width=150)
        existing_tree.column("时间", width=100)
        existing_tree.column("地点", width=100)
        existing_scroll = ttk.Scrollbar(existing_tree_frame, orient=tk.VERTICAL, command=existing_tree.yview)
        existing_tree.configure(yscrollcommand=existing_scroll.set)
        existing_tree.pack(side=tk.LEFT, fill=tk.X, expand=True)
        existing_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        
        for slot in tt.slots:
            existing_tree.insert("", tk.END, values=(
                slot.course_name,
                f"{DAY_NAMES.get(slot.day_of_week, '')} {slot.start_time}",
                slot.location or "-"
            ), tags=(slot.slot_id,))
        
        delete_frame = ttk.Frame(manage_row)
        delete_frame.pack(side=tk.LEFT, padx=5)
        
        def delete_selected():
            selection = existing_tree.selection()
            if not selection:
                messagebox.showwarning("提示", "请先选择要删除的课程", parent=dlg)
                return
            for item_id in selection:
                slot_id = existing_tree.item(item_id, "tags")[0] if existing_tree.item(item_id, "tags") else None
                if slot_id:
                    self._course_sim_engine.remove_slot(tt.timetable_id, slot_id)
            existing_tree.delete(*existing_tree.get_children())
            for slot in tt.slots:
                existing_tree.insert("", tk.END, values=(
                    slot.course_name,
                    f"{DAY_NAMES.get(slot.day_of_week, '')} {slot.start_time}",
            ...
            messagebox.showinfo("完成", f"已删除 {len(selection)} 门课程", parent=dlg)
        
        ttk.Button(delete_frame, text="🗑 删除选中", command=delete_selected).pack(pady=2)
        ttk.Button(delete_frame, text="🗑 清空全部", command=lambda: (
            [self._course_sim_engine.remove_slot(tt.timetable_id, s.slot_id) for s in tt.slots[:]],
            existing_tree.delete(*existing_tree.get_children()),
            messagebox.showinfo("完成", "已清空所有课程", parent=dlg)
        )).pack(pady=2)
        ...
        def do_sync():
            selected = [(cid, var.get()) for cid, var in check_vars.items() if var.get()]
            if not selected:
                messagebox.showwarning("提示", "请选择至少一门课程（双击勾选）", parent=dlg)
                return
            
            selected_days = [d for d, var in day_vars.items() if var.get()]
            if not selected_days:
                messagebox.showwarning("提示", "请选择至少一个星期", parent=dlg)
                return
            
            selected_periods = [p for p, var in period_vars.items() if var.get()]
            if not selected_periods:
                messagebox.showwarning("提示", "请选择至少一个节次", parent=dlg)
                return
            ...
            custom_hours = custom_hours_var.get()
            
            selected_titles = []
            for cid, _ in selected:
                for c in self.system.courses:
                    check_cid = c.get("note_id", c.get("course_title", "")) if isinstance(c, dict) else getattr(c, 'note_id', getattr(c, 'name', ''))
                    if check_cid == cid:
                        title = c.get("course_title", cid) if isinstance(c, dict) else getattr(c, 'name', cid)
                        selected_titles.append(title)
                        break
            
            days_str = ", ".join([DAY_NAMES.get(d, "")[:2] for d in selected_days])
            periods_count = len(selected_days) * len(selected_periods)
            total_slots = len(selected) * periods_count
            
            confirm_msg = f"即将同步以下内容：\n\n"
            confirm_msg += f"📚 课程：{len(selected)} 门\n"
            confirm_msg += f"   {', '.join(selected_titles[:3])}"
            if len(selected_titles) > 3:
                confirm_msg += f" 等{len(selected_titles)}门"
            confirm_msg += f"\n\n"
            confirm_msg += f"📅 星期：{days_str}\n"
            confirm_msg += f"⏰ 节次：{len(selected_periods)} 节/天\n"
            confirm_msg += f"📊 总计：约 {total_slots} 个课次\n"
            confirm_msg += f"📍 地点：{location or '未指定'}\n"
            confirm_msg += f"👨‍🏫 教师：{teacher or '未指定'}\n"
            confirm_msg += f"📆 学期：{start_date} 至 {end_date}\n\n"
            confirm_msg += f"确定要继续吗？"
            
            if not messagebox.askyesno("确认同步", confirm_msg, parent=dlg):
                return
            
            count = 0
            
            tt.semester_start = start_date
            tt.semester_end = end_date
            ...
            for cid, _ in selected:
                for c in self.system.courses:
                    check_cid = c.get("note_id", c.get("course_title", "")) if isinstance(c, dict) else getattr(c, 'note_id', getattr(c, 'name', ''))
                    if check_cid == cid:
                        course_objects[cid] = c
                        break
            
            for cid, _ in selected:
                data = course_data.get(cid, {})
                weekly_hours = custom_hours
                course_obj = course_objects.get(cid)
                
                course_content = ""
                course_summary = ""
                course_url = ""
                if course_obj and isinstance(course_obj, dict):
                    lessons = course_obj.get("lessons", [])
                    if lessons:
                        for lesson in lessons:
                            lesson_content = lesson.get("content", "")
                            lesson_title = lesson.get("title", "")
                            course_content += f"\n## {lesson_title}\n{lesson_content}\n"
                    domain = course_obj.get("domain", "")
                    course_summary = f"领域: {domain} | 周课时: {weekly_hours}节 | 教师: {teacher or data.get('teacher', '未指定')}"
                    course_url = course_obj.get("url", "")
                
                title = ""
                if course_obj and isinstance(course_obj, dict):
                    title = course_obj.get("course_title", cid)
                else:
                    title = getattr(course_obj, 'name', cid) if course_obj else cid
                
                for day in selected_days:
                    for p_idx in selected_periods:
                        p = PERIODS[p_idx]
                        
                        slot = TimetableSlot(
                            slot_id="",
                            course_id=cid,
                            course_name=title or cid,
                            day_of_week=day,
                            start_time=p[1],
                            end_time=p[2],
                            period_idx=p_idx,
                            location=location or data.get("location", ""),
                            teacher=teacher or data.get("teacher", ""),
                        )
                        self._course_sim_engine.add_slot(tt.timetable_id, slot)
                        count += 1
                
                if course_content and course_obj:
                    course_items_to_hub.append({
                        "id": cid,
                        "title": title or cid,
                        "content": course_content,
                        "summary": course_summary,
                        "url": course_url,
                        "domain": data.get("location", "") or getattr(course_obj, 'domain', '') if isinstance(course_obj, dict) else "",
                    })
            
            dlg.destroy()
            
            if course_items_to_hub:
                try:
                    from ws2_data_hub import get_data_hub, HubItem, SourceType, ItemType
                    hub = get_data_hub()
                    imported_count = 0
                    for item_data in course_items_to_hub:
                        hub_item = HubItem(
                            ...
                            tags=[item_data.get("domain", "课程"), "course", "timetable"],
                            metadata={
                                "course_id": item_data["id"],
                        ...
                        hub.add_item(hub_item)
                        imported_count += 1
                    messagebox.showinfo("同步完成", f"已添加 {count} 门课时到课表\n已导入 {imported_count} 个课程到数据中心")
                except Exception as e:
                    messagebox.showinfo("同步完成", f"已添加 {count} 门课时到课表\n(课程内容导入失败: {e})")
            else:
                self._show_course_simulation_page()
    ...
    def _update_course_monitor(self):
        try:
            ...
            if sim.is_in_course_mode:
                session = sim.active_session
                if session:
                    remaining = sim.get_remaining_time()
                    if remaining is not None:
                        h, m = divmod(remaining, 60)
                        self._course_monitor_var.set(f"🔴 课程进行中: {session.slot.course_name}")
                        self._course_countdown_var.set(f"剩余 {h:02d}:{m:02d}")
                    else:
                        self._course_monitor_var.set(f"🔴 课程进行中: {session.slot.course_name}")
                        self._course_countdown_var.set("")
            else:
                current = sim.get_current_slot()
                if current:
                    remaining = sim.get_remaining_time()
                    if remaining is not None:
                        h, m = divmod(remaining, 60)
                        self._course_monitor_var.set(f"🟡 当前有课: {current.course_name}")
                        self._course_countdown_var.set(f"剩余 {h:02d}:{m:02d}")
                else:
                    nxt = sim.get_next_slot()
                    if nxt:
                        day_name = DAY_NAMES.get(nxt.day_of_week, "")
                        self._course_monitor_var.set(f"🟢 空闲 | 下一节: {day_name} {nxt.start_time} {nxt.course_name}")
                        self._course_countdown_var.set("")
                    else:
                        self._course_monitor_var.set("🟢 空闲 | 本周无更多课程")
                        self._course_countdown_var.set("")

            self.root.after(10000, self._update_course_monitor)
        except Exception:
            pass
    ...
    def _add_timetable(self):
        ...
        ttk.Label(fields, text="课表名称:").grid(row=0, column=0, sticky=tk.W, pady=3)
        name_var = tk.StringVar(value="2026春季学期")
        ttk.Entry(fields, textvariable=name_var, width=25).grid(row=0, column=1, pady=3)

        ttk.Label(fields, text="学期开始:").grid(row=1, column=0, sticky=tk.W, pady=3)
        start_var = tk.StringVar(value="2026-02-17")
        ttk.Entry(fields, textvariable=start_var, width=25).grid(row=1, column=1, pady=3)

        ttk.Label(fields, text="学期结束:").grid(row=2, column=0, sticky=tk.W, pady=3)
        end_var = tk.StringVar(value="2026-06-30")
        ttk.Entry(fields, textvariable=end_var, width=25).grid(row=2, column=1, pady=3)

        ttk.Separator(fields, orient=tk.HORIZONTAL).grid(row=3, column=0, columnspan=2,
                                                          sticky=tk.EW, pady=10)
        ttk.Label(fields, text="添加课程时段:", font=("", 9, "bold")).grid(
            row=4, column=0, columnspan=2, sticky=tk.W)

        slots_frame = ttk.Frame(fields)
        slots_frame.grid(row=5, column=0, columnspan=2, sticky=tk.EW)

        slot_rows = []

        def add_slot_row():
            row_idx = len(slot_rows)
            rf = ttk.Frame(slots_frame)
            rf.pack(fill=tk.X, pady=2)

            day_var = tk.StringVar(value="1")
            ttk.Combobox(rf, textvariable=day_var, width=4,
                        values=[str(i) for i in range(1, 8)],
                        state="readonly").pack(side=tk.LEFT, padx=2)

            start_var = tk.StringVar(value="08:00")
            ttk.Entry(rf, textvariable=start_var, width=6).pack(side=tk.LEFT, padx=2)

            end_var = tk.StringVar(value="09:40")
            ttk.Entry(rf, textvariable=end_var, width=6).pack(side=tk.LEFT, padx=2)

            course_var = tk.StringVar()
            course_names = [c.name for c in self.system.courses] if hasattr(self, 'system') else []
            ttk.Combobox(rf, textvariable=course_var, width=15,
                        values=course_names).pack(side=tk.LEFT, padx=2)

            loc_var = tk.StringVar()
            ttk.Entry(rf, textvariable=loc_var, width=8).pack(side=tk.LEFT, padx=2)

            slot_rows.append((day_var, start_var, end_var, course_var, loc_var))

        add_slot_row()

        ttk.Button(fields, text="➕ 添加更多时段",
                   command=add_slot_row).grid(row=6, column=0, columnspan=2, pady=5)

        def do_add():
            try:
                from mcp.automation.course_simulation import Timetable, TimetableSlot
                slots = []
                for dv, sv, ev, cv, lv in slot_rows:
                    if cv.get():
                        slots.append(TimetableSlot(
                            slot_id=f"slot_{len(slots)}",
                            course_id=cv.get(),
                            course_name=cv.get(),
                            day_of_week=int(dv.get()),
                            start_time=sv.get(),
                            end_time=ev.get(),
                            location=lv.get()
                        ))
                tt = Timetable(
                    timetable_id=f"tt_{name_var.get().replace(' ', '_')}",
                    name=name_var.get(),
                    semester_start=start_var.get(),
                    semester_end=end_var.get(),
                    slots=slots
                )
                self._course_sim_engine.add_timetable(tt)
                self._show_course_simulation_page()
                dlg.destroy()
            except Exception as e:
        ...
        ttk.Button(btn_frame, text="保存", command=do_add).pack(side=tk.RIGHT, padx=5)
        ttk.Button(btn_frame, text="取消", command=dlg.destroy).pack(side=tk.RIGHT)

    def _check_current_course(self):
        if not hasattr(self, '_course_sim_engine'):
            return
        slot = self._course_sim_engine.get_current_slot()
        if slot:
            messagebox.showinfo("当前课程",
                f"📖 {slot.course_name}\n"
                f"⏰ {slot.start_time}-{slot.end_time}\n"
                f"📍 {slot.location}")
        else:
            messagebox.showinfo("当前课程", "🟢 当前无课程")

    def _enter_course_mode(self):
        if not hasattr(self, '_course_sim_engine'):
            return
        slot = self._course_sim_engine.get_current_slot()
        if not slot:
            messagebox.showwarning("提示", "当前时间段没有课程")
            return
        try:
            self._course_sim_engine.enter_course_mode(slot)
            course = self._get_course_detail(slot.course_id)
    ...
    def _exit_course_mode(self):
        if not hasattr(self, '_course_sim_engine'):
            return
        try:
            self._course_sim_engine.exit_course_mode()
            self._show_course_simulation_page()
        except Exception as e:
            messagebox.showerror("错误", str(e))

    def _open_learning_assistant(self):
        try:
            from mcp.automation.popup_manager import PopupManager
            pm = PopupManager.get_instance()
            pid = "learning_assistant"
            ...
            if not assistant:
                ...
                try:
                    assistant._window.lift()
                    assistant._window.focus_force()
                except Exception:
    ...
    def _init_automation_system(self):
        try:
            ...
            from mcp.automation.popup_manager import PopupManager
            from mcp.automation.course_simulation import CourseSimulationEngine
            
            # 尝试初始化 Agent/LLM 系统
            agent = None
            llm = None
            try:
                from mcp import agent as mcp_agent
                agent = mcp_agent.create_agent(
                    base_dir=BASE,
                ...
                import logging
                logging.getLogger(__name__).warning(f"Agent 初始化失败: {e}")

            if not hasattr(self, '_event_bus'):
                self._event_bus = EventBus()

            if not hasattr(self, '_popup_manager'):
                self._popup_manager = PopupManager.get_instance()
                if agent:
                    self._popup_manager.set_agent(agent)
                if llm:
                    self._popup_manager.set_llm(llm)
                self._popup_manager.set_ws2_system(self.system)

            if not hasattr(self, '_course_sim_engine'):
                self._course_sim_engine = CourseSimulationEngine(
                    root=self.root,
                    agent=agent,
                    llm=llm,
                    ws2_system=self.system
            ...
            self._event_bus.subscribe("course.*", self._on_course_event)

            self.root.after(5000, self._auto_popup_learning_assistant)
            self.root.after(60000, self._periodic_course_check)

        except Exception as e:
            import logging
            logging.getLogger(__name__).exception(f"自动化系统初始化失败: {e}")

    def _on_automation_event(self, event):
        try:
            from mcp.automation.popup_manager import PopupManager
            pm = PopupManager.get_instance()
            if event.event_type.startswith("automation.completed"):
                pm.push_to_assistant(
                    "learning_assistant",
                    "自动化任务完成",
                    str(event.data.get("message", "任务已执行"))
                )
            elif event.event_type.startswith("automation.failed"):
                pm.push_to_assistant(
                    "learning_assistant",
                    "自动化任务失败",
                    str(event.data.get("error", "未知错误"))
                )
        except Exception:
            pass
    ...
    def _on_workflow_event(self, event):
        try:
            ...
            pm.push_to_assistant(
                "learning_assistant",
                "工作流完成",
                f"工作流 {event.data.get('workflow_id', '')} 已完成"
            )
        except Exception:
            pass

    def save_session_record(self, session):
        """保存课程会话记录"""
        try:
            import json
            import os
            ...
            session_dir = os.path.join(os.path.expanduser("~"), ".ts2", "sessions")
            os.makedirs(session_dir, exist_ok=True)
            
            session_data = {
                "session_id": session.session_id if hasattr(session, 'session_id') else "",
                "slot": session.slot.to_dict() if hasattr(session.slot, 'to_dict') else {},
                "started_at": session.started_at if hasattr(session, 'started_at') else "",
                "status": session.status if hasattr(session, 'status') else "",
                "teacher_messages": session.teacher_messages if hasattr(session, 'teacher_messages') else [],
                "student_questions": session.student_questions if hasattr(session, 'student_questions') else [],
                "resources_opened": session.resources_opened if hasattr(session, 'resources_opened') else [],
                "notes_taken": session.notes_taken if hasattr(session, 'notes_taken') else "",
                "ended_at": datetime.now().isoformat(),
            }
            
            filename = f"session_{session.session_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            import logging
            logging.getLogger(__name__).error(f"Failed to save session record: {e}")

    def _on_course_event(self, event):
        try:
            from mcp.automation.popup_manager import PopupManager
            pm = PopupManager.get_instance()
    ...
    def _auto_popup_learning_assistant(self):
        try:
            from mcp.automation.popup_manager import PopupManager
            pm = PopupManager.get_instance()
            if not pm.get_assistant("learning_assistant"):
                assistant = pm.create_assistant("learning_assistant")
                assistant.show(parent=self.root)
                assistant.push_notification(
                    "📚 学习助手已启动",
                    "我会根据你的课程进度和学术动态自动推送提醒。\n"
                    "你也可以随时向我提问。"
                )
            self.root.after(2000, self._assistant_summarize_and_ask)
        except Exception:
            pass

    def _assistant_summarize_and_ask(self):
        try:
            from mcp.automation.popup_manager import PopupManager
            pm = PopupManager.get_instance()
            assistant = pm.get_assistant("learning_assistant")
            if not assistant or not assistant.is_alive():
                return
            ...
            try:
                if hasattr(self, '_course_sim_engine') and self._course_sim_engine:
                    slot = self._course_sim_engine.get_current_slot()
                    if slot:
                        summary_parts.append(
                            f"\n🎓 当前课程: {slot.course_name} "
                            f"({slot.start_time}-{slot.end_time})"
                        )
                    else:
                        summary_parts.append("\n🟢 当前无课程")
            ...
            try:
                from mcp.automation.engine import get_automation_engine
                engine = get_automation_engine()
                if engine:
                    auto_tasks = engine.list_tasks()
                    enabled = [t for t in auto_tasks if t.get("enabled", True)]
                    if enabled:
                        summary_parts.append(f"\n⏰ 自动化任务: {len(enabled)}项活跃")
                        for at in enabled[:3]:
                            summary_parts.append(f"  • {at.get('name', '')} [{at.get('trigger_type', '')}]")
            except Exception:
                pass

            try:
                if hasattr(self, 'system') and self.system:
                    courses = getattr(self.system, 'courses', [])
                    if courses:
                        total = len(courses)
                        completed = sum(
                            1 for c in courses
                            if getattr(c, 'progress', 0) >= 1.0
                        )
                        summary_parts.append(f"\n📊 课程进度: {completed}/{total}门已完成")
            except Exception:
                pass
            ...
            if summary_parts:
                ...
                assistant.show_message(
                    "assistant",
                    "以上是你当前的学习状态。有什么需要我帮你处理的吗？\n"
                    "例如：\n"
                    "• 查看某门课程的详细进度\n"
                    "• 提醒你完成待办任务\n"
                    "• 推荐学习资源\n"
    ...
    def _periodic_course_check(self):
        try:
            if hasattr(self, '_course_sim_engine'):
                self._course_sim_engine.check_and_trigger()
        except Exception:
            pass
        self.root.after(60000, self._periodic_course_check)
