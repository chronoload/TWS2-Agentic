...
使用腾讯云 API **AK/SK 签名认证**（TC3-HMAC-SHA256），通过环境变量配置：
...
当 AK/SK 未配置时（`check_env.py` 返回码 2），向用户展示：
...
```bash
source ~/.zshrc 2>/dev/null; source ~/.bashrc 2>/dev/null; python3 {baseDir}/check_env.py
```
...
```bash
python3 {baseDir}/check_env.py --enable-advisor
```
...
**所有用户问题统一通过 CloudQChatCompletions SSE 流式接口处理**：
...
脚本 `tcloud_sse_api.py` **已自动完成**控制台链接 → 免密登录链接的替换，AI 直接展示 `data.content` 即可。
...
| 接口 | 类型 | 所在脚本 | 调用条件 |
|------|------|---------|---------|
| `advisor:CloudQChatCompletions` | 只读（SSE） | `tcloud_sse_api.py` | 正常使用 |
