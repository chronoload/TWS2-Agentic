...
class SearchEngine:
    ...
    async def crawl_website(self, start_url: str, keywords: List[str] = None, save_pages: bool = True) -> List[Dict[str, Any]]:
        """
        爬取整个网站
        ...
        Returns:
            爬取结果列表
        """
        debug.log_function_call("SearchEngine.crawl_website", {"start_url": start_url[:50], "keywords": keywords, "save_pages": save_pages})
        ...
        logger.debug(f"爬取配置 - 最大深度: {max_depth}, 最大页数: {max_pages}")
        debug.log_data("爬取配置", f"深度:{max_depth}, 页数:{max_pages}, 并发:{self.config.get('max_concurrent', 10)}")
