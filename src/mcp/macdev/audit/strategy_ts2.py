"""外部项目策略示例（零特化演示）：TS2 语义表。
macdev 内核不含任何项目语义；本项目（或任意项目）通过继承 ChainStrategy
注册自己的类型/helper/别名/形参表，注册 `registry.register("audit.strategy", name, cls)`。
"""
...
class Ts2ChainStrategy(ChainStrategy):
    ...
    def collect_builtin_transforms(self) -> list:
        """示例：TS2 内置变换名（param_chain._BUILTIN_TRANSFORMS）。"""
        try:
