---
name: tapd-skill
description: "TAPD OpenAPI skill，用于需求、缺陷、任务、迭代、Wiki 搜索等 TAPD 平台操作。当用户提到「TAPD」「需求」「缺陷」「bug」「任务」「迭代」「Wiki」或需要在 TAPD 中查询、管理相关内容时，使用此 skill。支持需求管理、缺陷管理、任务管理、迭代管理、Wiki 管理等功能。"
...
| 服务 | 说明 | 常用 API 文件 |
|------|------|---------------|
| stories | 需求管理 | liststories, countstories, addstory, updatestory, getrelatedbugs |
| bugs | 缺陷管理 | listbugs, countbugs, addbug, updatebug |
