# 原子：scan_static_resources（原 interface_chain_extractor.py 第 2329 行）
# 逻辑组：scan · 由 scripts/split_extractor.py 机械生成，勿手改。
# 依赖整理与模块间 import 属上层抽象（superpowers spec/plan 流程），本文件不保证独立运行。
from __future__ import annotations

def scan_static_resources(root: Path, files: list, exclude: tuple = _SCAN_DEFAULT_EXCLUDE) -> list:
    items = []
    seen = set()
    for f in files:
        fp = Path(f)
