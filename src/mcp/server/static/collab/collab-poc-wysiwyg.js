...
function isPreviewNode(node, root) {
  ...
  while (current && current !== root) {
    if (current.classList && (
      current.classList.contains('vditor-ir__preview') ||
      current.classList.contains('vditor-wysiwyg__preview')
    )) return true;
...
function textNodes(root) {
  ...
  while ((node = walker.nextNode())) {
    if (!isPreviewNode(node, root) && node.nodeValue && node.nodeValue.replace(/\u200b/g, '').trim() !== '') nodes.push(node);
  }
  return nodes;
}
...
function buildMap(vd) {
  ...
  for (const node of textNodes(root)) {
    const chars = node.nodeValue.replace(/\u200b/g, '');
    if (!chars) continue;
...
function createVditor(id, value, after) {
  return new Vditor(id, {
    mode: 'wysiwyg', cdn: 'https://unpkg.com/vditor@3.10.7', height: '100%', theme: 'dark',
