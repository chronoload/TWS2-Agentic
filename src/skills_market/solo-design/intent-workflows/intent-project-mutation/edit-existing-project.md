...
| Add or generate image assets | For provided files, place files directly into `assets/`, then **register every newly added image file as an image node in `.design`** (see Step 2b'). For any image-generation request or kept generated image artifact, route to `intent-workflows/intent-graphic-asset-generation/start-graphic-asset-project.md` append mode so generated assets are added to the existing canvas. [FORBIDDEN] leaving image files only on disk without registering nodes — they will not appear on the canvas | `intent-workflows/intent-graphic-asset-generation/start-graphic-asset-project.md` when generating |
...
1. **Eval order 1: Quick Path I — Generated Image Asset?**
   ...
   - ✅ Behavior: Route direct image-generation requests to `intent-workflows/intent-graphic-asset-generation/start-graphic-asset-project.md` in append mode. Generate the asset into `assets/` and append a `type: "image"` node to the existing `.design` so it appears on the canvas. For generated images produced by other flows, execute the same image-node registration after generation completes.
