...
### 7) 子代理并行执行（superpowers dispatching-parallel-agents 整合，优先考虑）

**核心原则**：一个独立问题域派一个子代理，让它们并发工作（`sub_agent`）。子代理有**隔离上下文**——不继承主会话历史，由你精确构造所需上下文；这既保证它们聚焦成功，也**保留你的上下文用于协调**。

**子代理职能速查**（优先按职能选择，而非一律 general）：
- `coder`（编程/调试/重构，15 轮上限）— 实现/修 bug/重构
- `task`（完成特定任务，10 轮上限）— 需要跑命令/工具链的任务
- `research`（搜索/分析/总结，8 轮上限）— 并发调研/信息收集
- `review`（代码质量/安全审查，5 轮上限）— 独立审查视角

**并发优先原则（与铁律 13 audit 优先纪律衔接，探索与执行同样适用）**：
- **前提纪律（audit 先行）**：任何探索/追溯任务第一步**仍是 audit 建立全量上下文**（铁律 13：亲属追逐依赖链 + calls/refs 索引 + 缺陷全景，禁止 grep/read 零散起步）——子代理并行**不得替代 audit 起步**；audit 全量上下文就绪后，若存在 2+ 独立子域需深挖/修复/分块生成，再并行派发；
- **并发深入（audit 之后）**：全量上下文就绪后，2+ 独立问题域（不同子系统/文件/未知点）→ 并行派 research/coder 分域深入（含对多个独立目标并行 `dev audit --target`），比串行逐域推进快一个数量级；
- **并发执行**：2+ 独立任务（不同测试文件/独立修复/分块生成）→ 同一响应内多个 `sub_agent` 调用 = 并行执行（一次一个 = 串行）；
- **何时不并发**：任务相关（修一个可能修其他）、需要理解全系统状态、代理会互相干扰（改同一文件/用同一资源）——此时合并单代理或串行。

**Prompt 结构（缺一不可）**：①聚焦——一个明确问题域（"修复 X.test.ts"，不是"修复所有"）；②自包含——贴全错误信息/上下文/源文，不依赖主会话记忆；③约束——明确"不要改其他代码"等边界；④输出——明确要求返回"根因摘要 + 改动清单"，便于回审。

**回审整合（并发必做）**：读每个摘要 → 检查冲突（各代理是否改了同一文件）→ 跑全量验证 → 抽查（子代理可能有系统性错误，不可盲信）。

**实战注记**：长写入/大段重写易被对话流打断 → 用「副本 + 多子代理分块生成 + 主代理拼接/回审 + 拷贝回权威」模式（实例：§8 铁律 20→15 合并用 3 子代理并行分块 + Python 拼接，一次成功；产物收敛 `<name>-project/verify/`）。
