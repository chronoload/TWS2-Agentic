...
## 0. 强制前置（首次会话先读一次，读完才准动手）

先完整阅读两份使用文档（包内自包含：`macdev/doc/PLAN.md`、`macdev/doc/AUDIT_DOC.md`；
发布版在 `macdev-skill/` 同名文件）：

- **PLAN.md** — plan 命令全集：CRUD / verify / review / ledger / tdd / export / openspec
- **AUDIT_DOC.md** — audit 命令全集：audit task / chain 定向查询 / patch 自演化闭环
- **LOG_DOC.md** — log 经验沉淀命令全集：四分类 add/list/query/export
- **REQI_DOC.md** — requirement 命令全集：需求 CRUD + scan/align（双轨产物）
- **DEV_DOC.md** — dev 命令全集：学习/模仿目标项目（map 目录编排 + audit 架构了解）

> 读完才算加载完成。文档若缺失或过期：先 `python -m macdev doc` 重生成，再读。
> 开发文档（架构 + 各自模块函数索引）：AUDIT_DEV_DOC / PLAN_DEV / LOG_DEV / REQI_DEV / DEV_DEV
...
## 1. 流程编排（每次任务的固定步进，顺序执行）

1. **启发式关键词扫描（限定文件名与后缀，不盲目全扫）**
   - 先按「文件名含关键词 + 指定后缀」定点（如 `*skill*.py`、`*context*.py`），
   - 再按内容关键词收敛（`grep -l 'compress|summary|skill_manager' --include='*.py' <dir>`），
   - 限定目标目录与排除目录，得到候选文件集，不要全仓库瞎扫。

2. **编排审查逻辑与 task 参数**
   - 按目标写 AuditTask JSON：入口端点/规则表/目标目录/排除目录/策略名，
   - `python -m macdev audit --task task.json --root . --out out`，
   - 产物：INTERFACE_CHAIN.md + interface_chain.db + 全维度 CSV + events.ndjson。

3. **从产物了解架构链路（先整体，再细节）**
   - 先读 INTERFACE_CHAIN.md 的依赖链/亲属追逐图，回答「系统长什么样、谁依赖谁、断没断」，
   - db 可快速索引：`python -m macdev audit chain endpoints|issues|models|drifts|tables`
     做 SQL 级定向查询，替代 grep-read。

4. **回归语境核实（真实理解，不臆断）**
   - 宏观链图只给线索，动手前必须 `ls`/`grep`/`read` 回到语境核实：
     列出相关目录与文件 → grep 注入点/调用点 → read 关键函数体，
   - 特别核查上下文工程组件：记忆（memory/）、RAG 注入、ContextProvider 分层、
     工具注册方式、中间件链——这些不 ls 不 read 无法确认真实角色，
   - 链图上的 🔴 broken / 同名定义可能是参数/builtin 噪音，必须回源码甄别后再下结论。

5. **跟随链路审查**
   - 从入口沿依赖链逐段核对，查 db/csv 定位断裂点与缺陷，
   - 有疑问的功能定位也在此步完成（audit 即探索）。

6. **可选写 patch（自演化闭环）**
   - `patch gen` → `patch plugins` → `patch apply` → 重 audit → `patch verify` 对比缺陷收敛。

7. **进入开发流程（grilling/spec → plan → 再审查）**
   - openspec 桥接（list/status/import/export/sync-specs）沉淀 spec，
   - plan 提供全套接口：`plan create/task/step` + `plan verify`（完成门禁）+ `plan ledger`（resume），
   - 实施后**重审计**对比缺陷收敛，闭环收尾。

```bash
# 编排一屏流
python -m macdev audit --task task.json --root . --out out      # 2. 审查
python -m macdev audit chain endpoints --db out/interface_chain.db  # 3. 快速索引
python -m macdev patch gen --db out/interface_chain.db --root . --out patches/   # 5. 可选
python -m macdev plan create --title "..." --goal "..."          # 6. 开发流程
