...
- write_file：写入文件，支持 create/append 模式
- edit_file：编辑文件，支持5种模式：replace(搜索替换), insert(行号前插入), append(末尾追加), delete_lines(删除行范围), undo(恢复备份)
...
class Agent:
    def __init__(
        ...
        if HAS_SUBAGENT:
            try:
                from .config import get_config_manager
                self._coordinator = Coordinator(llm=self.llm)
                            ...
                            try:
                                if hasattr(self, '_multi_provider'):
                                    ...
                                    if hasattr(self.llm, 'config'):
                                        sub_llm = LLM(
                                            provider_config=self.llm.config.model_copy(
                                                update={'model': cfg.model}
                                            )
                                        )
                            except Exception as e:
...
def create_agent(
    ...
    # 将 multi_provider_manager 传递给 agent，用于子 Agent 创建
